...
void iLuxEntity::UpdatePlayerLookAt(float afTimeStep)
{
	...
	// Coop: try P1 first, then P2 if P1 is not looking at it.
	//
	// mbIsLookedAt below stays a SINGLE flag on purpose -- the callback fires
	// once when the FIRST player looks, and the leave edge fires once when
	// NEITHER is looking. A one-shot story beat can therefore never double-fire
	// because both players happened to glance at the same thing, and
	// mbLookAtCallbackRemove keeps clearing it exactly as before.
	cLuxPlayer *vLookPlayers[2] = { gpBase->mpPlayer, gpBase->mpPlayer2 };
	cLuxPlayer *pLookingPlayer = NULL;

	bool bLookingAt=false;
	for(int lLookPass=0; lLookPass<2 && bLookingAt==false; ++lLookPass)
	{
		cLuxPlayer *pLookPlayer = vLookPlayers[lLookPass];
		if(pLookPlayer == NULL) continue;
		if(lLookPass == 1 && ImGuiDebugMenu::GetP2LookAtCallbacks()==false) continue;

		cCamera *pCamera = pLookPlayer->GetCamera();
		...
		if(bLookingAt) pLookingPlayer = pLookPlayer;
	}
	...
	if(bLookingAt && mbIsLookedAt == false)
	{
		...
		// Coop: script commands act on gpBase->mpPlayer, so the player who
		// actually looked gets the sequence rather than whoever is P1.
		mpCoopLookAtPlayer = pLookingPlayer;
		cLuxPlayer *pPrevActing = cLuxPlayer::CoopBeginActAs(pLookingPlayer);
		mpMap->RunScript(sTempCallback + "(\""+msName+"\", 1)" );
		cLuxPlayer::CoopEndActAs(pPrevActing);
	}
	else if(bLookingAt==false && mbIsLookedAt)
	{
		// The leave edge belongs to whoever triggered the enter edge -- but that
		// player may be gone by now (coop switched off mid-map deletes P2 while
		// the entity lives on). The only two live players are ever mpPlayer and
		// mpPlayer2, so anything else is a stale pointer: drop it rather than
		// hand freed memory to the script.
		cLuxPlayer *pLeavePlayer = mpCoopLookAtPlayer;
		if(pLeavePlayer != gpBase->mpPlayer && pLeavePlayer != gpBase->mpPlayer2) pLeavePlayer = NULL;

		cLuxPlayer *pPrevActing = cLuxPlayer::CoopBeginActAs(pLeavePlayer);
...
{
	// Defaults to P1 so every pre-co-op caller keeps its old behaviour.
	return CollidesWithPlayer(gpBase->mpPlayer);
}

//-----------------------------------------------------------------------

bool iLuxEntity::CollidesWithPlayer(cLuxPlayer *apPlayer)
{
	iPhysicsWorld *pPhysicsWorld = mpMap->GetPhysicsWorld();
	cCollideData collideData;
	collideData.SetMaxSize(1);

	// P2 is created and destroyed by hand around map changes, so it can be
	// NULL here even while co-op is on -- and a player mid-teardown has no
	// character body. Both are normal, not errors.
	if(apPlayer == NULL || apPlayer->GetCharacterBody() == NULL) return false;

	iPhysicsBody *pPlayerBody = apPlayer->GetCharacterBody()->GetCurrentBody();
