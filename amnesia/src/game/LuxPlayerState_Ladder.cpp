...
void cLuxPlayerState_Ladder::OnEnterState(eLuxPlayerState aPrevState)
{
	...
	/////////////////////////////////////
	// No ladder to attach to -- LoadFromSaveDataBeforeEnter leaves mpLadder NULL
	// when the saved area ID no longer resolves (edited map, custom story), and
	// the next line would dereference it. Bail out to Normal instead of crashing.
	// Deliberately placed HERE: gravity, collision, the lantern and the pitch
	// limits are all set above, so the OnLeaveState that ChangeState runs undoes
	// a fully-initialised state rather than reading garbage.
	if(mpLadder == NULL)
	{
