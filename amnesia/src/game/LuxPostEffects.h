...
public:	
	...
	/**
	 * Coop: register an effect for per-frame Update() WITHOUT attaching it to
	 * the main viewport's composite. The split-screen composites carry their
	 * own per-player insanity instances (cLuxMapHandler builds them); their
	 * wave/zoom values are per player but the animation clock still has to
	 * tick, and this handler owns that loop.
	 */
	void AddCoopUpdateEffect(iLuxPostEffect *apPostEffect);
