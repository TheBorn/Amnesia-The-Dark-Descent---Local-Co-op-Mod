...
{
	// Detect which player is active: if gpBase->mpPlayer was swapped to P2
	// during interaction, route the message to P2.
	int lPlayerIdx = 0;
	if (gpBase->mpPlayer2 && gpBase->mpPlayer == gpBase->mpPlayer2)
		lPlayerIdx = 1;

	SetMessage(asText, afTime, lPlayerIdx);
}

void cLuxMessageHandler::SetMessage(const tWString& asText, float afTime, int alPlayerIndex)
{
