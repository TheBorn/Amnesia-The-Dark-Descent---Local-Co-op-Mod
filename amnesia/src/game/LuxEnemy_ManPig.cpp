...
bool cLuxEnemy_ManPig::StateEventImplement(int alState, eLuxEnemyStateEvent aEvent, cLuxStateMessage *apMessage)
{
	...
	kLuxState(eLuxEnemyState_Alert)
		...
		kLuxOnEnter
			...
			else
			{
				...
				GetTargetPlayer()->AddTerrorEnemy(this);
		...
		kLuxOnUpdate
			...
			if(mbThreatenOnAlert && PlayerIsDetected())
			{
				float fAngleDist = cMath::GetAngleDistanceRad(	mpCharBody->GetYaw()+kPif, 
																GetTargetPlayer()->GetCharacterBody()->GetYaw());
			...
			if(GetTargetPlayer()->GetTerror() >= 1 && mbStuckAtDoor)// && mpMap->DoorIsClosed(mlStuckDoorID))
			{
				iLuxEntity *pDoorEnt = mpMap->GetEntityByID(mlStuckDoorID);
				mvTempPos = pDoorEnt->GetAttachEntity()->GetWorldPosition();
				ChangeState(eLuxEnemyState_BreakDoor);
			...
			{
				float fTerror = GetTargetPlayer()->GetTerror();
				if (mbIsTelsa == true)
				{
			...
			else
			{
				if(mbThreatenOnAlert)
				{
					...
					else
					{
						...
						if( (GetTargetPlayer()->GetTerror() >= 1 && ( mfAlertRunTowardsCount>mfAlertRunTowardsToHuntLimit || fDistToPlayer < mfAlertToInstantHuntDistance*2)) ||
							fDistToPlayer < mfAlertToInstantHuntDistance)
						{
							GetTargetPlayer()->SetTerror(1.0f);
	...
	kLuxState(eLuxEnemyState_Search)
		...
		kLuxOnMessage(eLuxEnemyMessage_TimeOut_2)
			...
			cAINode * pNode = mpPathfinder->GetNodeAtPos(GetTargetPlayer()->GetCharacterBody()->GetFeetPosition(), 4, 12,false, false, true, NULL);
			if(pNode)
				mpPathfinder->MoveTo(pNode->GetPosition());
			else
				ChangeState(eLuxEnemyState_Patrol);
	...
	kLuxState(eLuxEnemyState_HuntPause)	
		...
		kLuxOnMessage(eLuxEnemyMessage_TimeOut)
			...
			if(DistToPlayer2D() > 2.0f)
				mpPathfinder->MoveTo(GetTargetPlayer()->GetCharacterBody()->GetFeetPosition());
			else
				mpMover->TurnToPos(GetTargetPlayer()->GetCharacterBody()->GetFeetPosition());
...
bool cLuxEnemy_ManPig::TrackTeleportBehindPlayer()
{
	...
	cVector3f vPlayerBackward = GetTargetPlayer()->GetCharacterBody()->GetForward()*-1;
...
{
	cFrustum *pFrust = GetTargetPlayer()->GetCamera()->GetFrustum();

	//Log("Check inside ray:\n");

	/////////////////////////
	//If near enough, just check if in frustum
	if(cMath::Vector3Dist(mpCharBody->GetPosition(), GetTargetPlayer()->GetCharacterBody()->GetPosition())<2.25f)
	{
		//Log(" true: close!\n");
...
{
	if(GetTargetPlayer()->GetHelperLantern()->IsActive()==false || mbDisableTriggers)
	{
...
void cLuxEnemy_ManPig::ResetMindFuckEffects()
{
	...
	GetTargetPlayer()->FadeAspectMulTo(1.0f, 10.0f);
	GetTargetPlayer()->FadeFOVMulTo(1.0f,1.0f);
	mbTeslaMindFuckActive = false;
	mfTeslaMindFuckPulse =0;
	mfTeslaMindFuckPulseAdd=1;
...
{
	if(GetTargetPlayer()->IsDead() || IsActive()==false)
	{
	...
	// Special Notice Checks
	{
		...
		float fSpeed = GetTargetPlayer()->GetAvgSpeed();
		if(fSpeed>fMinSpeed)
		{
			float fMul = 0.15f;
			float fDistance = DistToPlayer2D();
	...
	// Effects
	{
		...
		if(mfTeslaSpecialNoticeCount>0.0f)
		{
			...
			////////////////////////
			// Aspect Mul
			//GetTargetPlayer()->FadeAspectMulTo(1.0f + mfTeslaMindFuckPulse*mfTeslaSpecialNoticeCount*0.3f, 10.0f);
			//GetTargetPlayer()->FadeFOVMulTo(1.0f - mfTeslaMindFuckPulse*mfTeslaSpecialNoticeCount*0.3f, 10.0f);
