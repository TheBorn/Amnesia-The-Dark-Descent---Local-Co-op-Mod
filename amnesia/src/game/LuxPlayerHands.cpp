...
cLuxPlayerHandsLoader* cLuxPlayerHands::spSharedHandsLoader = NULL;
cLuxHandObjectLoader* cLuxPlayerHands::spSharedHandObjectLoader = NULL;
...
cLuxPlayerHands::cLuxPlayerHands(cLuxPlayer *apPlayer) : iLuxPlayerHelper(apPlayer, "LuxPlayerHands")
{
	...
	if(spSharedHandsLoader == NULL)
	{
		// First instance (P1) — register the entity loaders
		spSharedHandsLoader = hplNew( cLuxPlayerHandsLoader, ("PlayerHands", this) );
		gpBase->mpEngine->GetResources()->AddEntityLoader(spSharedHandsLoader);

		spSharedHandObjectLoader = hplNew( cLuxHandObjectLoader, ("HandObject") );
		gpBase->mpEngine->GetResources()->AddEntityLoader(spSharedHandObjectLoader);
	}

	// Each player gets their own reference to the shared hand object loader
	mpHandObjectLoader = spSharedHandObjectLoader;
...
{
	// Only the first instance (P1) owns the mesh and animation resources
...
void cLuxPlayerHands::SetVisualsVisibleForCamera(bool abOwnView)
{
	//CRASH GUARD. This runs from the viewport PRE-DRAW callback -- the same place
	//cLuxPlayerAvatar and cLuxPlayerGun are driven from, and the same place the
	//gun crashed. So it can fire in the window between a world being destroyed
	//and this helper being told about it: quit to menu and level changes destroy
	//the map directly without ever reaching DestroyWorldEntities, and a frame can
	//still be rendered in between.
	//
	//Everything below then dereferences freed memory -- mpHandsEntity->IsVisible()
	//and every hand object's entity. The avatar has guarded this for a while and
	//the gun now does; the hands were the one sibling left.
	//
	//The player having no character body is the tell: the same teardown destroys
	//it, and the gun's crash backtrace showed exactly that (mpCharBody = 0x0).
	if(mpPlayer == NULL || mpPlayer->GetCharacterBody() == NULL) return;
	if(gpBase->mpMapHandler->GetCurrentMap() == NULL) return;

	// The game's own intent: hands show unless the state machine has them
	// fully holstered/disabled.
	bool bGameWantsHands = (mHandState != eLuxHandsState_Disabled);
	bool bShowHands = abOwnView && bGameWantsHands;

	if(mpHandsEntity && mpHandsEntity->IsVisible() != bShowHands)
	{
		mpHandsEntity->SetVisible(bShowHands);
	}

	// Hand objects: only the CURRENT one may show, and only in the owner's
	// view. Lights are untouched (see SetModelVisible).
	for(size_t i=0; i<mvHandObjects.size(); ++i)
	{
		iLuxHandObject *pObject = mvHandObjects[i];
		bool bCurrent = (pObject == mpCurrentHandObject) && bGameWantsHands;
		pObject->SetModelVisible(abOwnView && bCurrent);
	}
}
...
void cLuxPlayerHands::CreateHandEntity(cLuxMap *apMap)
{
	...
	// Temporarily redirect the shared entity loader to THIS hands instance
	// so the AfterLoad callback stores the entity in the correct player's hands.
	cLuxPlayerHands *pOrigTarget = NULL;
	if(spSharedHandsLoader)
	{
		pOrigTarget = NULL; // We don't have a getter, but SetTarget overwrites it
		spSharedHandsLoader->SetTargetHands(this);
	}

	// Use unique entity name so P2's hands don't collide with P1's
	tString sEntityName = mpPlayer->IsPlayer2() ? "PlayerHands_P2" : "PlayerHands";
	apMap->GetWorld()->CreateEntity(sEntityName, cMatrixf::Identity, "models/player/hands/hands.ent");
	
	// Restore loader target to P1 (the original owner)
	if(spSharedHandsLoader && mpPlayer->IsPlayer2() && gpBase->mpPlayer && gpBase->mpPlayer->GetHands())
	{
		spSharedHandsLoader->SetTargetHands(gpBase->mpPlayer->GetHands());
	}
...
void cLuxPlayerHands::UpdatePlayerHandsPos(float afTimeStep)
{
	...
	cVector3f vFinalPos = pCam->GetPosition() - vHandPosAdd * mfPosAddMul;

	mtxHands.SetTranslation(vFinalPos);
