...
void cLuxArea_SlimeDamage::OnUpdate(float afTimeStep)
{
	...
	//////////////////////////
	// Check collision
	// Co-op: slime is a world volume, so whoever is standing in it takes the
	// hit. CollidesWithPlayer() only ever tested P1 (LuxEntity.cpp), so P2
	// waded through slime untouched -- and when P2 was the ONLY one in it the
	// area did nothing at all: no damage, no sound, no shake, no callback.
	cLuxPlayer *vPlayers[2] = { gpBase->mpPlayer, gpBase->mpPlayer2 };

	bool bAnyHit = false;
	for(int i=0; i<2; ++i)
	{
		if(vPlayers[i] == NULL) continue;
		if(CollidesWithPlayer(vPlayers[i]) == false) continue;

		bAnyHit = true;

		/////////////////////////
		//Damage player. Rolled per player, so two people in the same pool do
		//not take an identical hit. GiveDamage works entirely off 'this'
		//(health, hud effect, death helper), so it is safe for P2.
		float fDamage = cMath::RandRectf(mfMinAttackDamage, mfMaxAttackDamage);
		vPlayers[i]->GiveDamage(fDamage,1,eLuxDamageType_BloodSplat, true, false);
	}

	//////////////////////////
	// Everything below is per-ATTACK, not per-player: one sound, one particle
	// burst, one script callback, however many players got splashed.
	if(bAnyHit)
	{
		/////////////////////////
		//Sound
		...
		/////////////////////////
		//ScreenShake. cLuxEffect_ShakeScreen is a single global effect rather
		//than a per-player one, so this fires once for the attack instead of
		//once per player hit. Making the shake per-player is a separate change.
