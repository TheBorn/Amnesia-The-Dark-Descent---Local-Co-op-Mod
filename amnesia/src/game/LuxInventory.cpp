...
{
	std::vector<cLuxInventory_Item*>& vItems = mpInventory->GetActiveItems();
	if((int)vItems.size() <= mlIdx) 
		return NULL;
	else
		return vItems[mlIdx];
...
cLuxPlayer* cLuxInventory::GetActivePlayer()
{
	// If explicitly set (inventory is open), use that
	if (mpActivePlayer) return mpActivePlayer;
	// During P2's interaction, gpBase->mpPlayer is temporarily swapped to P2
	if (gpBase->mpPlayer2 && gpBase->mpPlayer == gpBase->mpPlayer2)
		return gpBase->mpPlayer2;
	return gpBase->mpPlayer;
}

int cLuxInventory::GetActivePlayerIndex()
{
	return GetActivePlayer()->IsPlayer2() ? 1 : 0;
}

std::vector<cLuxInventory_Item*>& cLuxInventory::GetActiveItems()
{
	if (ImGuiDebugMenu::GetFullItemShare()) return mvItems;
	if (GetActivePlayer()->IsPlayer2()) return mvItemsP2;
	return mvItems;
}

cLuxInventory_Item*& cLuxInventory::GetActiveEquippedItem()
{
	if (ImGuiDebugMenu::GetFullItemShare()) return mpEquippedItem;
	if (GetActivePlayer()->IsPlayer2()) return mpEquippedItemP2;
	return mpEquippedItem;
}

//-----------------------------------------------------------------------

bool cLuxInventory::PlayerHasItemOfType(cLuxPlayer *apPlayer, eLuxItemType aType)
{
	if(apPlayer==NULL) return false;

	// Deliberately does NOT call GetActiveItems(): that resolves its list from
	// mpActivePlayer (the player whose inventory was last opened), which is not
	// the same question. With Full Item Share on there is only one list, so both
	// players answer from it.
	std::vector<cLuxInventory_Item*>& vItems =
		(ImGuiDebugMenu::GetFullItemShare()==false && apPlayer->IsPlayer2()) ? mvItemsP2 : mvItems;

	for(size_t i=0; i<vItems.size(); ++i)
	{
		if(vItems[i]->GetType() == aType) return true;
	}
...
bool cLuxInventory::AnyPlayerHasItemOfType(eLuxItemType aType)
{
	for(size_t i=0; i<mvItems.size(); ++i)   if(mvItems[i]->GetType()==aType)   return true;
	for(size_t i=0; i<mvItemsP2.size(); ++i) if(mvItemsP2[i]->GetType()==aType) return true;
	return false;
}
...
{
	////////////////////////////
	// Coop: tick the world so the other player keeps playing.
	//
	// Driven from HERE, straight out of this menu's own Update. This module is
	// the current container, so if this line is reached the world ran -- there is
	// no flag to be stale, no registration to be missed, no ordering to get wrong.
	// The previous approach set a "background container" on the updater and every
	// value read back correct while the world sat frozen anyway.
	//
	// Single player still pauses.
	if(gpBase->mpMapHandler->GetCoopMode())
	{
		gpBase->mpEngine->GetUpdater()->RunMessageOnContainer("Default",
														eUpdateableMessage_Update, afTimeStep);
	}
	...
	//
	// Coop: ask the device that OWNS this bag. eLuxAction_UIPrimary is gamepad A
	// as well as Return, so reading it raw meant P2 simply HOLDING jump pinned an
	// item to P1's cursor, and P2 letting go dropped it out from under them.
	if(gpBase->mpInputHandler->MenuActionHeld(eLuxAction_LeftClick)==false && 
		gpBase->mpInputHandler->MenuActionHeld(eLuxAction_UIPrimary)==false)
	{
		mpPickedItem = NULL;
	}
	...
	else
	{
		...
		if(mfAlpha < 0)
		{
			...
			if(mbExitToJournal)
			{
				...
				//Coop: carry the owner across, or the journal lands on P1's half.
				gpBase->mpJournal->SetActivePlayer(GetActivePlayer());
...
{
	////////////////////////
	// The inventory always has a player context while open. Default to P1
	// when entered without one (e.g. returning from the journal, which
	// clears the context on the way out).
	if (mpActivePlayer == NULL) mpActivePlayer = gpBase->mpPlayer;

	////////////////////////
	//If player has use item state, change that
	if(GetActivePlayer()->GetCurrentState() == eLuxPlayerState_UseItem)
	{	
		GetActivePlayer()->ChangeState(eLuxPlayerState_Normal);
	}
	...
	// In coop, position the inventory viewport at the OPENING player's
	// viewport area — both players get an in-viewport inventory while the
	// other player's half keeps rendering live.
	if (gpBase->mpMapHandler->GetCoopMode())
	{
		cViewport *pPlayerVp = GetActivePlayer()->IsPlayer2() ?
			gpBase->mpMapHandler->GetCoopViewport() :
			gpBase->mpMapHandler->GetCoopP1Viewport();
		if (pPlayerVp)
		{
			mpViewport->SetPosition(pPlayerVp->GetPosition());
			mpViewport->SetSize(pPlayerVp->GetSize());
		}
	}

	mpViewport->SetActive(true);

	//The world running behind this is asserted every frame by cLuxInputHandler.
	mpViewport->SetVisible(true);
	mpGuiSet->SetActive(true);

#ifdef USE_GAMEPAD
	//Which UI mode this menu opens in. "Is a gamepad plugged in" is the wrong
	//question in coop -- one always is, because it is P2's, so P1's bag opened in
	//gamepad mode: cursor hidden, mouse movement off, focus highlight on. That
	//focus highlight jumping between slots is the snapping. Ask who OWNS the menu
	...
	/////////////////////
	//Effects
	//Coop: the world is still running for the other player, so none of the
	//quieting a paused game wants applies. Voices are the loud one -- a flashback
	//is a world event both players are living through, and pausing it cut the
	//line off mid-sentence for the player who never opened anything. Same for
	//ducking world volume and fading out looping sounds: there is a whole player
	//out there listening to it.
	//
	//The RESTORES on the way out are left alone. Restoring volume that was never
	//ducked is a no-op, and it means a bag opened before this build -- or in
	//single player -- always ends up back at full volume.
	bool bQuietTheWorld = (gpBase->mpMapHandler->GetCoopMode()==false);

	if(bQuietTheWorld)
		gpBase->mpMapHandler->GetCurrentMap()->GetPhysicsWorld()->FadeoutAllLoopSounds(1.0f);

	//Their own heartbeat, and they can still be caught while browsing.
	if(bQuietTheWorld)
		GetActivePlayer()->StopTerrorSound();

	///////////////////////////
	//Pause voices and turn down volume on world sounds
	if(bQuietTheWorld && mbEnterFromJournal == false)
	{
...
#if USE_GAMEPAD
	//Only seed nav focus for a menu that is actually being driven by a pad.
	if(mpLastCurrentWidget && bPadUI)
	{
...
void cLuxInventory::OnLeaveContainer(const tString& asNewContainer)
{
	...
	// Clear active player context when closing inventory
	mpActivePlayer = NULL;
...
void cLuxInventory::OnDraw(float afFrameTime)
{
	...
	//No overlay on the other player's half. Dimming P2 made sense while opening a
	//bag paused the world; now that it does not, it just turns the lights down on
	//someone who is still walking around.
	...
	if(mpPickedItem)
	{
		//Ownership-aware, like the drop check in Update. Raw, this asks whether ANY
		//device is holding the button -- harmless only because LeftClick happens to
		//be mouse-only and non-configurable, which is not a guarantee worth resting
		//on inside a menu that can belong to either player.
		cVector3f vPickedItemPos = mvPickedItemOffset;
		if(gpBase->mpInputHandler->MenuActionHeld(eLuxAction_LeftClick))
	...
	mpGuiSet->DrawFont(	mpFontDefault, vPlayerDataStart+cVector3f(fPlayerDataXAdd,0,0),20,cColor(1,mfAlpha),_W("%.0f%%"), GetActivePlayer()->GetLampOil());
	...
	mpGuiSet->DrawFont(	mpFontDefault, vPlayerDataStart+cVector3f(fPlayerDataXAdd,0,0),20,cColor(1,mfAlpha),_W("%d"), GetActivePlayer()->GetCoins());
	vPlayerDataStart.y+= 22;

	mpGuiSet->DrawFont(	mpFontDefault, vPlayerDataStart,20,cColor(1,mfAlpha),_W("%ls"), kTranslate("Inventory", "Tinderboxes:").c_str());
	mpGuiSet->DrawFont(	mpFontDefault, vPlayerDataStart+cVector3f(fPlayerDataXAdd,0,0),20,cColor(1,mfAlpha),_W("%d"), GetActivePlayer()->GetTinderboxes());
	vPlayerDataStart.y+= 22;
	*/
											...
											bool* apRemoveItemProp)
{
	...
	////////////
	// Add item to inventory — route to the correct player's list.
	// During P2's interaction, gpBase->mpPlayer is temporarily swapped to P2.
	if(apRemoveItemProp) *apRemoveItemProp = true;
	// mbForceAddToPlayer2 exists for savegame loading: the swap idiom below needs a
	// LIVE cLuxPlayer for P2, but items are restored before P2 is constructed, so
	// the swap cannot be used there. See SetForceAddToPlayer2().
	//
	// The swap is right for a WORLD pickup, where P2's interaction points
	// gpBase->mpPlayer at P2. It is wrong for anything a script does while the
	// inventory is OPEN -- combining, above all. That runs from the inventory UI
	// with no swap in force, so gpBase->mpPlayer was still P1 and the combined
	// item landed in P1's bag after P2 had spent their own two pieces making it.
	//
	// mpActivePlayer is set when a bag is opened and cleared when it closes, so
	// it is non-NULL only inside exactly that window -- world pickups are
	// unaffected and still resolve through the swap.
	cLuxPlayer *pOwner = mpActivePlayer;
	if(pOwner == NULL && gpBase->mpPlayer2 && gpBase->mpPlayer == gpBase->mpPlayer2)
		pOwner = gpBase->mpPlayer2;

	if (mbForceAddToPlayer2 || (pOwner && pOwner->IsPlayer2()))
		(ImGuiDebugMenu::GetFullItemShare() ? mvItems : mvItemsP2).push_back(pItem);
	else
...
void cLuxInventory::RemoveItem(const tString& asName)
{
	...
	// Search P2's items
	it = mvItemsP2.begin();
	for(; it != mvItemsP2.end(); ++it)
	{
		cLuxInventory_Item *pItem = *it;
		if(pItem->GetName() == asName)
		{
			if(mpPickedItem == pItem) mpPickedItem = NULL;
			if(mpEquippedItemP2 == pItem) mpEquippedItemP2 = NULL;
...
void cLuxInventory::RemoveItem(cLuxInventory_Item *apItem)
{
	...
	if(mpEquippedItemP2 == apItem) mpEquippedItemP2 = NULL;
...
{
	// Search active player's items first, then the other
	std::vector<cLuxInventory_Item*>& vActive = GetActiveItems();
	for(size_t i=0; i<vActive.size(); ++i)
	{
		cLuxInventory_Item *pItem = vActive[i];
...
int cLuxInventory::GetItemIndex(cLuxInventory_Item *apItem)
{
	...
	for(size_t i=0; i<vActive.size(); ++i)
	{
		if(vActive[i] == apItem) return (int)i;
	}
	return -1;
}
...
{
	// Search both lists — scripts may look up items by name without player context.
	// Whoever has a bag OPEN is searched first: a combine script removes its parts
	// by name, and if both players carry an item of the same name the combining
	// player must spend their own copy, not the other player's. With no bag open
	// this is P1 then P2, exactly as before.
	bool bP2First = (mpActivePlayer && mpActivePlayer->IsPlayer2());

	std::vector<cLuxInventory_Item*> *pLists[2];
	pLists[0] = bP2First ? &mvItemsP2 : &mvItems;
	pLists[1] = bP2First ? &mvItems : &mvItemsP2;

	for(int l=0; l<2; ++l)
	{
		std::vector<cLuxInventory_Item*> &vList = *pLists[l];
		for(size_t i=0; i<vList.size(); ++i)
		{
			if(vList[i]->GetName() == asName) return vList[i];
		}
	}
	return NULL;
}
...
{
	// During interaction, gpBase->mpPlayer is swapped — check active player's items first
	std::vector<cLuxInventory_Item*>& vActive = GetActiveItems();
	for(size_t i=0; i<vActive.size(); ++i)
	{
		if(vActive[i]->GetSubType() == asSubType) return vActive[i];
	}
	return NULL;
}
...
bool cLuxInventory::HasItemOfType(eLuxItemType aType)
{
	...
	for(size_t i=0; i<vActive.size(); ++i)
	{
		if(vActive[i]->GetType() == aType) return true;
	}
	return false;
}
...
{
	cLuxInventory_Item*& pEquipped = GetActiveEquippedItem();
	if(pEquipped == apItem) return;
	pEquipped = apItem;
}

//-----------------------------------------------------------------------

cLuxInventory_Item* cLuxInventory::GetEquippedHandItem()
{
	if (ImGuiDebugMenu::GetFullItemShare()) return mpEquippedItem;
	if (mpActivePlayer && mpActivePlayer->IsPlayer2()) return mpEquippedItemP2;
	return mpEquippedItem;
}
...
{
	//Coop: a pad being present says nothing about whose bag this is. P1 owns
	//theirs with a mouse, and turning the pad focus highlight on for them on any
	//hotplug event undid what OnEnterContainer set up. Same rule as there.
	bool bPadUI = gpBase->mpInputHandler->IsGamepadPresent();
...
void cLuxInventory::RenderBackgroundImage()
{
	...
	//////////////////////////////
	// Render
	// NOTE: when P2's inventory is open, RenderBackgroundScreen (via the map
	// handler's P2Full capture mode) renders P2's view alone, full-screen.
	// All viewport/camera juggling for that lives in
	// cLuxMapHandler::Prepare/RestoreBackgroundCapture — not here.
...
{
	int lSanityStatus = StatusToIndex(GetActivePlayer()->GetSanity());
...
{
	int lHealthStatus = StatusToIndex(GetActivePlayer()->GetHealth());
...
bool cLuxInventory::OilOnDraw(iWidget* apWidget, const cGuiMessageData& aData)
{
	...
	float fRemainingOilHeight = GetActivePlayer()->GetLampOil()*0.01f * vFullOilSize.y;
...
bool cLuxInventory::TinderboxOnDraw(iWidget* apWidget, const cGuiMessageData& aData)
{
	...
	mpGuiSet->DrawFont( mpFontDefault, vTinderboxCenter+cVector3f(0,-vCountFontSize.y*0.5f,1), 
						...
						_W("x %d"), GetActivePlayer()->GetTinderboxes());
