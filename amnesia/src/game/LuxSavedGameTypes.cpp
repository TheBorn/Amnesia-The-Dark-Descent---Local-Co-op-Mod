...
void cLuxEffectHandler_SaveData::FromEffectHandler(cLuxEffectHandler *apEffects)
{
	...
	//Save format unchanged: P1's half, restored onto both. A fade caught mid-flight
	//by a save is rare, and restoring both halves is the safe direction -- it can
	//never bring a save back with one player's screen stuck black.
	mfFade_GoalAlpha = pFade->mfGoalAlpha[0];
	mfFade_Alpha = pFade->mfAlpha[0];
	mfFade_FadeSpeed = pFade->mfFadeSpeed[0];
...
void cLuxEffectHandler_SaveData::ToEffectHandler(cLuxMap *apMap, cLuxEffectHandler *apEffects)
{
	...
	pFade->mfGoalAlpha[0] = pFade->mfGoalAlpha[1] = mfFade_GoalAlpha;
	pFade->mfAlpha[0]     = pFade->mfAlpha[1]     = mfFade_Alpha;
	pFade->mfFadeSpeed[0] = pFade->mfFadeSpeed[1] = mfFade_FadeSpeed;
...
{
	mbDisabled = false;
	mlEquippedHandItem = -1;
	mlEquippedHandItemP2 = -1;
}
...
void cLuxInventory_SaveData::FromInventory(cLuxInventory *apInventory)
{
	...
	///////////////////////////
	// Coop: Player 2's items + equipped hand item (index into P2's list)
	mvItemsP2.Resize(apInventory->mvItemsP2.size());
	for(size_t i=0; i<mvItemsP2.Size(); ++i)
	{
		mvItemsP2[i].FromItem(apInventory->mvItemsP2[i]);
	}

	mlEquippedHandItemP2 = -1;
	for(size_t i=0; i<apInventory->mvItemsP2.size(); ++i)
	{
		if(apInventory->mvItemsP2[i] == apInventory->mpEquippedItemP2)
		{
			mlEquippedHandItemP2 = (int)i;
			break;
		}
	}
...
void cLuxInventory_SaveData::ToInventory(cLuxMap *apMap, cLuxInventory *apInventory)
{
	...
	// NOTE: P1's equipped hand item is restored BELOW, after the item list is
	// populated. Doing it here indexed GetActiveItems() before the items were
	// added -> out-of-range vector access -> crash on load (notably after a
	// coop menu round-trip, where the active list is empty or stale).
	...
	///////////////////////////
	// Coop: Player 2's items.
	//
	// This used to require gpBase->mpPlayer2 to be alive, because it routed items
	// by swapping gpBase->mpPlayer to P2 the way P2 interactions do. But the
	// inventory is restored BEFORE cLuxMapHandler::CreatePlayer2 runs, so mpPlayer2
	// was still NULL and the whole block was skipped -- P2 came out of every save
	// with an empty bag, and the lantern simply vanished from the game if P2 was
	// carrying it. Route with an explicit flag instead, which needs no live P2.
	if(mvItemsP2.Size() > 0)
	{
		apInventory->SetForceAddToPlayer2(true);

		for(size_t i=0; i<mvItemsP2.Size(); ++i)
		{
			cLuxInventory_Item_SaveData& saveItem = mvItemsP2[i];
		...
		apInventory->SetForceAddToPlayer2(false);
	}

	// P2's equipped hand item (direct — index into P2's list)
	if(mlEquippedHandItemP2 >= 0 && mlEquippedHandItemP2 < (int)apInventory->mvItemsP2.size())
		apInventory->mpEquippedItemP2 = apInventory->mvItemsP2[mlEquippedHandItemP2];
	else
		apInventory->mpEquippedItemP2 = NULL;

	// P1's equipped hand item (index into P1's list), restored here after the
	// items exist so the index cannot run past the end of the vector.
	if(mlEquippedHandItem >= 0 && mlEquippedHandItem < (int)apInventory->mvItems.size())
		apInventory->mpEquippedItem = apInventory->mvItems[mlEquippedHandItem];
	else
		apInventory->mpEquippedItem = NULL;
...
kSerializeVar(mlEquippedHandItemP2, eSerializeType_Int32)
kSerializeClassContainer(mvItemsP2, cLuxInventory_Item_SaveData, eSerializeType_Class)
kEndSerialize()
...
cLuxSaveGame_SaveData::cLuxSaveGame_SaveData()
{
	...
	// Coop defaults — old saves without these fields load as single player.
	mbCoopActive = false;
...
////////////////////////
// Coop
kSerializeVar(mbCoopActive, eSerializeType_Bool)
kSerializeVar(mbCoopStorySupport, eSerializeType_Bool)
kSerializeVar(mbCoopOptCrouchBoost, eSerializeType_Bool)
kSerializeVar(mbCoopOptPlayerCollision, eSerializeType_Bool)
kSerializeVar(mbCoopOptMonsterKilling, eSerializeType_Bool)
kSerializeVar(mfCoopOptMonsterDamageMul, eSerializeType_Float32)
kSerializeVar(mbCoopOptGlobalLantern, eSerializeType_Bool)
kSerializeVar(mlCoopSplitMode, eSerializeType_Int32)
kSerializeVar(mlCoopDualMonitor, eSerializeType_Int32)
kSerializeVar(mbHasPlayer2, eSerializeType_Bool)
kSerializeVar(mPlayer2, eSerializeType_Class)
