...
void cLuxPostEffectHandler::AddCoopUpdateEffect(iLuxPostEffect *apPostEffect)
{
	//Update ticking only -- the caller attaches it to whichever composite it
	//belongs to. Starts inactive like everything else here.
	mvPostEffects.push_back(apPostEffect);
	apPostEffect->SetActive(false);
}
