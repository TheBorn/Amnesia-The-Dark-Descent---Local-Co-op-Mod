...
public:	
	...
	bool AnyPlayerHasItemOfType(eLuxItemType aType);  // Coop: check BOTH players' lists

	/**
	 * Coop: does THIS SPECIFIC player hold an item of the given type?
	 *
	 * Use this instead of HasItemOfType() whenever the question is about a
	 * particular player rather than "the player whose inventory is on screen".
	 * HasItemOfType() goes through GetActiveItems(), which picks its list from
	 * mpActivePlayer -- i.e. whoever last OPENED the inventory -- so asking it
	 * about P2 while P1 was the last to open a bag silently answers for P1.
	 */
	bool PlayerHasItemOfType(cLuxPlayer *apPlayer, eLuxItemType aType);

	/**
	 * Force AddItem() to route into P2's list without a live cLuxPlayer for P2.
	 *
	 * Normally AddItem decides by the gpBase->mpPlayer swap idiom, which requires
	 * gpBase->mpPlayer2 to already exist. Savegame loading restores the inventory
	 * BEFORE P2 is constructed, so the old restore was guarded on mpPlayer2 and
	 * silently threw away every one of P2's items when it was still NULL -- P2
	 * loaded a save with an empty bag. Always pair set(true) with set(false).
	 */
	void SetForceAddToPlayer2(bool abX){ mbForceAddToPlayer2 = abX; }
	...
	cLuxInventory_Item* GetItem(int alIdx){ std::vector<cLuxInventory_Item*>& v = GetActiveItems(); return (alIdx >= 0 && alIdx < (int)v.size()) ? v[alIdx] : NULL; }
	int GetItemNum(){ return (int)GetActiveItems().size();}
	
	void SetEquippedHandItem(cLuxInventory_Item *apItem);
	cLuxInventory_Item* GetEquippedHandItem();  // Returns active player's equipped item

	// Per-player inventory support
	void SetActivePlayer(cLuxPlayer* apPlayer){ mpActivePlayer = apPlayer; }
	cLuxPlayer* GetActivePlayer();

	// True while the inventory has an explicit player context (set on open,
	// cleared on close) — used to tell inventory captures apart from
	// full-screen menu captures.
	bool HasPlayerContext(){ return mpActivePlayer != NULL; }

	cViewport* GetViewport(){ return mpViewport; }
	std::vector<cLuxInventory_Item*>& GetActiveItems();
	cLuxInventory_Item*& GetActiveEquippedItem();
...
private:
	...
	std::vector<cLuxInventory_Item*> mvItems;       // P1's items
	std::vector<cLuxInventory_Item*> mvItemsP2;     // P2's items
	
	cLuxPlayer* mpActivePlayer;  // Which player's inventory is currently shown
	bool mbForceAddToPlayer2;    // Savegame restore: route AddItem to P2 before P2 exists
	...
	cLuxInventory_Item *mpEquippedItem;     // P1's equipped hand item
	cLuxInventory_Item *mpEquippedItemP2;   // P2's equipped hand item
