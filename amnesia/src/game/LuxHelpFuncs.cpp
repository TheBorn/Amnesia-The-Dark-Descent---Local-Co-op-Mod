...
{
	cLuxMapHandler *pMapHandler = gpBase->mpMapHandler;

	// The map handler is the single owner of viewport state during a capture.
	// It decides what the snapshot contains: Composite (full-screen menu in
	// split-screen: both views + letterbox bars), P1Full/P2Full (a player's
	// inventory: that player's view alone, full-screen), DualP1 (dual monitor:
	// P1's screen only) or None (no coop).
	cLuxMapHandler::eBackgroundCaptureMode captureMode = pMapHandler->GetBackgroundCaptureMode();

	pMapHandler->PrepareBackgroundCapture();

	// Only in None mode (no coop) does this function toggle the main viewport
	// around the capture — show for the render, hide after, because the menu
	// covers the screen with the snapshot. All coop modes (P1Full/P2Full/
	// DualP1) manage viewport state inside Prepare/RestoreBackgroundCapture;
	// in DualP1 the main viewport must STAY visible after the capture, since
	// dual-monitor menus draw only a dim and P1's monitor needs the live
	// world rendering beneath the UI.
	bool bMainViewportCapture =
		captureMode == cLuxMapHandler::eBackgroundCaptureMode_None;

	// In composite mode, clear to black first so areas outside the two
	// viewports (the letterbox bars) are captured black, not stale frames.
	if (captureMode == cLuxMapHandler::eBackgroundCaptureMode_Composite)
	{
		iLowLevelGraphics *pLowGfx = gpBase->mpEngine->GetGraphics()->GetLowLevel();
		pLowGfx->SetClearColor(cColor(0, 0));
		pLowGfx->ClearFrameBuffer(eClearFrameBufferFlag_Color | eClearFrameBufferFlag_Depth | eClearFrameBufferFlag_Stencil);
	}

	if (bMainViewportCapture)
	{
		pMapHandler->GetViewport()->SetVisible(true);
	}
...
#ifdef COOP_CAPTURE_DEBUG
	// Diagnostic: dump the exact state the capture renders with.
	{
		cViewport *pDbgP1 = pMapHandler->GetCoopP1Viewport();
		cViewport *pDbgP2 = pMapHandler->GetCoopViewport();
		cVector2f vDbgScreen = gpBase->mpEngine->GetGraphics()->GetLowLevel()->GetScreenSizeFloat();
		Log("[COOP-CAP] capMode=%d splitMode=%d HUD=%d screen=(%d,%d) mainVis=%d\n",
			(int)captureMode, pMapHandler->GetSplitScreenMode(), (int)abDrawFullHUD,
			(int)vDbgScreen.x, (int)vDbgScreen.y, (int)pMapHandler->GetViewport()->IsVisible());
		if(pDbgP1) Log("[COOP-CAP] P1vp vis=%d pos=(%d,%d) size=(%d,%d) cam=%d world=%d\n",
			(int)pDbgP1->IsVisible(), (int)pDbgP1->GetPosition().x, (int)pDbgP1->GetPosition().y,
			(int)pDbgP1->GetSize().x, (int)pDbgP1->GetSize().y, pDbgP1->GetCamera()!=NULL, pDbgP1->GetWorld()!=NULL);
		else Log("[COOP-CAP] P1vp = NULL\n");
		if(pDbgP2) Log("[COOP-CAP] P2vp vis=%d pos=(%d,%d) size=(%d,%d) cam=%d world=%d\n",
			(int)pDbgP2->IsVisible(), (int)pDbgP2->GetPosition().x, (int)pDbgP2->GetPosition().y,
			(int)pDbgP2->GetSize().x, (int)pDbgP2->GetSize().y, pDbgP2->GetCamera()!=NULL, pDbgP2->GetWorld()!=NULL);
		else Log("[COOP-CAP] P2vp = NULL\n");
	}
#endif
	...
	//NOT cleared here.
	//
	// This capture runs in the middle of a frame, from OnEnterContainer, and the
	// objects in this set were built moments earlier by cGui::OnDraw for the frame
	// that has not been rendered yet. Clearing them threw that frame's HUD away:
	// cScene::Render then walked an empty set and drew nothing, so the HUD blinked
	// out for exactly one frame every time a menu, inventory or journal opened.
	//
	// It bought nothing either way -- cGui::OnPostBufferSwap clears every set at the
	// end of the frame regardless. Rendering the same objects twice is harmless: the
	// capture's copy is overwritten by the real frame, and the clip regions they
	// point at now outlive both passes.

	if (bMainViewportCapture)
	{
		pMapHandler->GetViewport()->SetVisible(false);
	}

	// Restore viewports to the state OnLeaveContainer expects
	...
	//
	// In a Composite capture the deferred renderer's final blit for the LAST
	// coop viewport ends with SetCurrentFrameBuffer(NULL, <P2 rect pos>, <P2
	// rect size>), and the low-level layer caches that offset. Every caller of
	// this function immediately does CopyFrameBufferToTexure(...), which ADDS
	// the cached position to its source coordinates — so the copy read from
	// P2's rect offset (partially outside the backbuffer) and filled the
	// inventory/journal/pause background textures with garbage. Explicitly
	// rebinding at (0,0)/full-screen zeroes the cached offset and also resets
	// glViewport for the raw post-processing quads that follow.
	iLowLevelGraphics *pLowGfxReset = gpBase->mpEngine->GetGraphics()->GetLowLevel();
	pLowGfxReset->SetCurrentFrameBuffer(NULL, cVector2l(0, 0), pLowGfxReset->GetScreenSizeInt());
}
