...
{
	gpBase->mpEffectRenderer->ClearOutlineObjects(mpPlayer->GetPlayerIndex());
...
void iLuxPlayerState_DefaultBase::Update(float afTimeStep)
{
	...
	// Use corrected ray direction for split-screen (viewport center, not camera center)
	cVector3f vRayDir = mpPlayer->GetInteractionForward();

	gpBase->mpMapHelper->GetClosestEntity(	pCam->GetPosition(), vRayDir, 20.0f, 
											&mfFocusDistance,&mpBodyInFocus,&mpEntityInFocus);
	mvFocusPos = pCam->GetPosition() + vRayDir*mfFocusDistance;
...
bool iLuxPlayerState_DefaultBase::OnDoAction(eLuxPlayerAction aAction,bool abPressed)
{
	...
	if(aAction == eLuxPlayerAction_Interact)
	{
		...
		if(abPressed)
		{
			...
			{
				// Coop: a level door requires BOTH players nearby, unless the debug
				// toggle relaxes it to a single player.
				if(gpBase->mpMapHandler->GetCoopMode() && !ImGuiDebugMenu::GetLevelDoorSinglePlayer() &&
				   mpEntityInFocus->GetEntityType()==eLuxEntityType_Prop &&
				   static_cast<iLuxProp*>(mpEntityInFocus)->GetPropType()==eLuxPropType_LevelDoor)
				{
					cLuxPlayer *pOtherLD = mpPlayer->IsPlayer2() ? gpBase->mpPlayer : gpBase->mpPlayer2;
					if(pOtherLD && pOtherLD->GetCharacterBody())
					{
						float fRadLD = cMath::Max(mpEntityInFocus->GetMaxFocusDistance()*1.5f, 3.5f);
						if(cMath::Vector3Dist(pOtherLD->GetCharacterBody()->GetPosition(), mvFocusPos) > fRadLD)
						{
							gpBase->mpMessageHandler->SetMessage(_W("The other player must be at the door too."), 0, mpPlayer->GetPlayerIndex());
							return false;
						}
					}
				}

				// If the debug menu option is OFF ("Allow Grabbing Same Object" unchecked),
				// block grabbing objects already held by the other player.
				if(!ImGuiDebugMenu::GetAllowGrabSameObject())
				{
					cLuxPlayer *pOtherPlayer = mpPlayer->IsPlayer2() ? gpBase->mpPlayer : gpBase->mpPlayer2;
					if(pOtherPlayer && pOtherPlayer->IsGrabbingEntity(mpEntityInFocus))
					{
						return false;
					}
				}

				// Entity OnInteract code uses gpBase->mpPlayer to change player state.
				// For P2 interactions, temporarily swap the global pointer so the entity
				// changes the correct player's state.
				// Routed through the shared helper so the displaced player is
				// recorded -- script teleports fired from in here still move BOTH.
				cLuxPlayer *pOrigPlayer = cLuxPlayer::CoopBeginActAs(mpPlayer);

				mpEntityInFocus->OnInteract(mpBodyInFocus,mvFocusPos);

				// The map script's PlayerInteractCallback. One-shot callbacks clear
				// themselves inside RunInteractCallbackFunc, so the second player
				// using the same object never re-fires it.
				if(mpPlayer->IsPlayer2()==false || ImGuiDebugMenu::GetP2InteractCallbacks())
				{
					mpEntityInFocus->RunInteractCallbackFunc();
				}

				// Restore
				cLuxPlayer::CoopEndActAs(pOrigPlayer);
			}
			return false;
		}
	}
...
void iLuxPlayerState_DefaultBase::AddOutlineObjects(iPhysicsBody *apBody, iLuxEntity *apEntity, const cVector3f &avFocusPos)
{
	...
	if(pProp->ShowOutlinesOnConnectedBodies())
	{
		...
		for(int i=0; i<pMeshEntity->GetSubMeshEntityNum(); ++i)
		{
			...
			{
				gpBase->mpEffectRenderer->AddOutlineObject(pSubEnt, mpPlayer->GetPlayerIndex());
			}
		}
	}
	/////////////////////////////
