...
void cLuxJournal::Update(float afTimeStep)
{
	...
	else
	{
		...
		if(mfAlpha<0)
		{
			...
			{
				//Carry the owner back. The outbound leg does this; the return leg did
				//not, and the inventory NULLs its active player on leave and falls back
				//to P1 on enter. So P2 going bag -> diary -> back landed in a P1-owned
				//inventory: locked out of the menu they opened, with P1's keyboard
				//driving it.
				if(GetActivePlayer())
					gpBase->mpInventory->SetActivePlayer(GetActivePlayer());
...
void cLuxJournal::OnEnterContainer(const tString& asOldContainer)
{
	...
	////////////////////////////
	// Coop: sit on the opening player's half so the other one can keep playing
	// behind it. A note picked up in the world is the exception -- that goes to
	// both, full screen, exactly as it always did.
	if(gpBase->mpMapHandler->GetCoopMode() && mbShowOnBothPlayers==false)
	{
	...
	{
		//Full screen. Set explicitly: the viewport is persistent, so a previous
		//per-player open would otherwise leave it stuck on half the screen.
	...
	if(bQuietTheWorld && mbOpenedFromInventory == false)
	{
	...
	/////////////////////
	//Dual-monitor coop: mirror the GUI onto P2's monitor so both players can read
	//it -- but ONLY when it is meant for both. This dates from the journal being a
	//full-screen pause menu; now that a journal belongs to whoever opened it, an
	//unconditional mirror is exactly what put P1's notes on P2's screen.
	//
	//The mirror is still right for a world pickup: in dual monitor the full-screen
	//viewport only covers the primary monitor, so P2 needs this to see it at all.
	if(mbShowOnBothPlayers)
		gpBase->mpMapHandler->AddDualMenuMirror(mpGuiSet);
}
...
void cLuxJournal::OnLeaveContainer(const tString& asNewContainer)
{
	...
	////////////////////////////
	//Set up states
	gpBase->mpMapHandler->RemoveDualMenuMirror(mpGuiSet);
...
void cLuxJournal::OnDraw(float afFrameTime)
{
	...
	//Draw background
	if (gpBase->mpMapHandler->GetCoopMode() &&
		gpBase->mpMapHandler->GetSplitScreenMode() == cLuxMapHandler::eSplitScreenMode_DualMonitor)
	{
		// Dual-monitor coop: the captured snapshot is P1's view, and since
		// this GUI set is mirrored onto P2's monitor a snapshot background
		// would show P1's view on BOTH screens. Draw a translucent dim
		// instead — each monitor's own (frozen) world viewport keeps
		// rendering beneath it, so both players see the journal/note over
		// their own view.
		mpGuiSet->DrawGfx(mpWhiteGfx, mvGuiSetStartPos+cVector3f(0,0,0.2f), mvGuiSetSize, cColor(0, 0.6f*mfAlpha));
	}
	else
	{
