...
private:
	...
	*/
	...
	//////////////////////////////
	// Coop
	static int __stdcall GetTriggerPlayer();
	static bool __stdcall GetCoopActive();

	// Coop Options -- story-scriptable, settable at any time (see the
	// implementations for what each does). In a native coop story these ARE
	// the effective values; under forced coop the debug menu's toggles rule.
	static void __stdcall SetCoopAllowCrouchBoost(bool abX);
	static bool __stdcall GetCoopAllowCrouchBoost();
	static void __stdcall SetCoopAllowPlayerCollision(bool abX);
	static bool __stdcall GetCoopAllowPlayerCollision();
	static void __stdcall SetCoopAllowMonsterKilling(bool abX);
	static bool __stdcall GetCoopAllowMonsterKilling();
	static void __stdcall SetCoopMonsterPropDamageMul(float afMul);
	static float __stdcall GetCoopMonsterPropDamageMul();
	static void __stdcall SetCoopGlobalLantern(bool abX);
	static bool __stdcall GetCoopGlobalLantern();

	//Not generated by kLuxCoopPlayerVariantsDecl: these do not go through the
	//acting-player swap. A shake is aimed by handing the effect a target index, and
	//the unsuffixed one has to keep shaking BOTH players in a story that never
	//declared co-op support -- which a swap could not express.
	static void __stdcall StartScreenShake1(float afAmount, float afTime, float afFadeInTime,float afFadeOutTime);
	static void __stdcall StartScreenShake2(float afAmount, float afTime, float afFadeInTime,float afFadeOutTime);

	//////////////////////////////
	// Coop: per-player variants of every script function that acts on a player.
	//
	//   Foo()   the acting player -- whoever triggered the chain of events.
	//   Foo1()  Player 1 only.
	//   Foo2()  Player 2 only; does nothing when there is no Player 2.
	//
	//Generated rather than written out because there are 62 of them and the
	//wrapper is identical every time -- see kLuxCoopPlayerVariantsVoid in the cpp.
#define kLuxCoopPlayerVariantsDecl(RET, NAME, PARAMS) \
	static RET __stdcall NAME##1 PARAMS; \
	static RET __stdcall NAME##2 PARAMS;

	kLuxCoopPlayerVariantsDecl(void, CheckPoint, (string& asName,string& asStartPos ,string& asCallback, string &asDeathHintCat, string &asDeathHintEntry))
	kLuxCoopPlayerVariantsDecl(void, SetInDarknessEffectsActive, (bool abX))
	kLuxCoopPlayerVariantsDecl(bool, GetFlashbackIsActive, ())
	kLuxCoopPlayerVariantsDecl(void, StartPlayerSpawnPS, (string& asSPSFile))
	kLuxCoopPlayerVariantsDecl(void, SetPlayerActive, (bool abActive))
	kLuxCoopPlayerVariantsDecl(void, ChangePlayerStateToNormal, ())
	kLuxCoopPlayerVariantsDecl(void, SetPlayerCrouching, (bool abCrouch))
	kLuxCoopPlayerVariantsDecl(void, AddPlayerBodyForce, (float afX, float afY, float afZ, bool abUseLocalCoords))
	kLuxCoopPlayerVariantsDecl(void, ShowPlayerCrossHairIcons, (bool abX))
	kLuxCoopPlayerVariantsDecl(void, SetPlayerPos, (float afX, float afY, float afZ))
	kLuxCoopPlayerVariantsDecl(float, GetPlayerPosX, ())
	kLuxCoopPlayerVariantsDecl(float, GetPlayerPosY, ())
	kLuxCoopPlayerVariantsDecl(float, GetPlayerPosZ, ())
	kLuxCoopPlayerVariantsDecl(void, SetPlayerSanity, (float afSanity))
	kLuxCoopPlayerVariantsDecl(void, AddPlayerSanity, (float afSanity))
	kLuxCoopPlayerVariantsDecl(float, GetPlayerSanity, ())
	kLuxCoopPlayerVariantsDecl(void, SetPlayerHealth, (float afHealth))
	kLuxCoopPlayerVariantsDecl(void, AddPlayerHealth, (float afHealth))
	kLuxCoopPlayerVariantsDecl(float, GetPlayerHealth, ())
	kLuxCoopPlayerVariantsDecl(void, SetPlayerLampOil, (float afOil))
	kLuxCoopPlayerVariantsDecl(void, AddPlayerLampOil, (float afOil))
	kLuxCoopPlayerVariantsDecl(float, GetPlayerLampOil, ())
	kLuxCoopPlayerVariantsDecl(float, GetPlayerSpeed, ())
	kLuxCoopPlayerVariantsDecl(float, GetPlayerYSpeed, ())
	kLuxCoopPlayerVariantsDecl(void, MovePlayerForward, (float afAmount))
	kLuxCoopPlayerVariantsDecl(void, SetPlayerPermaDeathSound, (string& asSound))
	kLuxCoopPlayerVariantsDecl(void, SetSanityDrainDisabled, (bool abX))
	kLuxCoopPlayerVariantsDecl(void, GiveSanityBoost, ())
	kLuxCoopPlayerVariantsDecl(void, GiveSanityBoostSmall, ())
	kLuxCoopPlayerVariantsDecl(void, GiveSanityDamage, (float afAmount, bool abUseEffect))
	kLuxCoopPlayerVariantsDecl(void, GivePlayerDamage, (float afAmount, string& asType, bool abSpinHead, bool abLethal))
	kLuxCoopPlayerVariantsDecl(void, FadePlayerFOVMulTo, (float afX, float afSpeed))
	kLuxCoopPlayerVariantsDecl(void, FadePlayerAspectMulTo, (float afX, float afSpeed))
	kLuxCoopPlayerVariantsDecl(void, FadePlayerRollTo, (float afX, float afSpeedMul, float afMaxSpeed))
	kLuxCoopPlayerVariantsDecl(void, MovePlayerHeadPos, (float afX, float afY, float afZ, float afSpeed, float afSlowDownDist))
	kLuxCoopPlayerVariantsDecl(void, StartPlayerLookAt, (string& asEntityName, float afSpeedMul, float afMaxSpeed, string &asAtTargetCallback))
	kLuxCoopPlayerVariantsDecl(void, StopPlayerLookAt, ())
	kLuxCoopPlayerVariantsDecl(void, SetPlayerMoveSpeedMul, (float afMul))
	kLuxCoopPlayerVariantsDecl(void, SetPlayerRunSpeedMul, (float afMul))
	kLuxCoopPlayerVariantsDecl(void, SetPlayerLookSpeedMul, (float afMul))
	kLuxCoopPlayerVariantsDecl(void, SetPlayerJumpForceMul, (float afMul))
	kLuxCoopPlayerVariantsDecl(void, SetPlayerJumpDisabled, (bool abX))
	kLuxCoopPlayerVariantsDecl(void, SetPlayerCrouchDisabled, (bool abX))
	kLuxCoopPlayerVariantsDecl(void, SetPlayerFallDamageDisabled, (bool abX))
	kLuxCoopPlayerVariantsDecl(void, TeleportPlayer, (string &asStartPosName))
	kLuxCoopPlayerVariantsDecl(void, SetLanternActive, (bool abX, bool abUseEffects))
	kLuxCoopPlayerVariantsDecl(bool, GetLanternActive, ())
	kLuxCoopPlayerVariantsDecl(void, SetLanternDisabled, (bool abX))
	kLuxCoopPlayerVariantsDecl(void, SetDeathHint, (string &asTextCategory, string &asTextEntry))
	kLuxCoopPlayerVariantsDecl(void, DisableDeathStartSound, ())
	kLuxCoopPlayerVariantsDecl(void, FadeIn, (float afTime))
	kLuxCoopPlayerVariantsDecl(void, FadeOut, (float afTime))
	kLuxCoopPlayerVariantsDecl(void, StartEffectEmotionFlash, (string &asTextCat, string &asTextEntry, string &asSound))
	kLuxCoopPlayerVariantsDecl(void, SetMessage, (string &asTextCategory, string &asTextEntry, float afTime))
	kLuxCoopPlayerVariantsDecl(void, StartRandomInsanityEvent, ())
	kLuxCoopPlayerVariantsDecl(void, StartInsanityEvent, (string& asEventName))
	kLuxCoopPlayerVariantsDecl(void, StopCurrentInsanityEvent, ())
	kLuxCoopPlayerVariantsDecl(void, GiveItem, (string& asName, string& asType, string& asSubTypeName, string& asImageName, float afAmount))
	kLuxCoopPlayerVariantsDecl(void, GiveItemFromFile, (string& asName, string& asFileName))
	kLuxCoopPlayerVariantsDecl(void, RemoveItem, (string& asName))
	kLuxCoopPlayerVariantsDecl(bool, HasItem, (string& asName))
	kLuxCoopPlayerVariantsDecl(void, ExitInventory, ())
};
