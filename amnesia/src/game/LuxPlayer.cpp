...
// Coop: set while gpBase->mpPlayer is pointed at someone other than its real
// owner, so CoopGetSwappedOutPlayer() can hand the displaced player back.
static cLuxPlayer *gpCoopSwappedOutPlayer = NULL;

//-----------------------------------------------------------------------

cLuxPlayer::cLuxPlayer(int alPlayerIndex) : iLuxUpdateable("LuxPlayer"), iLuxCollideCallbackContainer()
{
	//////////////////////////////////
	// Init data pointers
	mpCharBody = NULL;
	mpTerrorSound = NULL;
	mlPlayerIndex = alPlayerIndex;
	...
	mbNoclipEnabled = false;
	mbNoclipActive = false;
	mbNoclipKeyWasDown = false;
	mfNoclipSpeed = 5.0f;
	mfNoclipFastMul = 3.0f;
	mfNoclipSlowMul = 0.25f;
	...
	// Both players get their own hands instance. The entity loader is shared
	// (global singleton) but redirected to the correct instance during creation.
	mpHands = hplNew( cLuxPlayerHands, (this) );
	mvHelpers.push_back(mpHands);

	mpGun = hplNew( cLuxPlayerGun, (this) );
	mvHelpers.push_back(mpGun);

	// Built for both players; every entry point returns early for P2.
	mpPossess = hplNew( cLuxPlayerPossess, (this) );
	mvHelpers.push_back(mpPossess);

	//////////////////////////////////
	// Co-op avatar (P1 = blue, P2 = red) — drawn only in the other player's view.
	mpAvatar = hplNew( cLuxPlayerAvatar, (this,
		alPlayerIndex == 1 ? cColor(1.0f, 0.25f, 0.2f, 1.0f) : cColor(0.2f, 0.45f, 1.0f, 1.0f)) );
...
cLuxPlayer::~cLuxPlayer()
{
	...
	// Destroy the camera that was created in the constructor. For P1 this only
	// happens at shutdown (engine/scene still alive at that point); for P2 it
	// happens on every coop teardown, so without this the camera leaked each
	// time. Any viewport referencing this camera must have been detached first
	// (DestroyPlayer2 nulls the coop viewport's camera before deleting P2).
	if(mpCamera)
	{
		gpBase->mpEngine->GetScene()->DestroyCamera(mpCamera);
		mpCamera = NULL;
	}
}
...
// Runtime debug control: sanity drain from darkness.
// Stored as an inverse flag ("disabled") for convenience in the codebase.
...
{
	////////////////////////
	// Noclip V-key toggle and movement
	UpdateNoclip(afTimeStep);

	////////////////////////
	// Co-op avatar gait animation
	if(mpAvatar) mpAvatar->Update(afTimeStep);
	...
	if(!mbNoclipActive && mpCharBody->GetPosition().y < mfAutoKillYPos) SetHealth(0);
			
	////////////////////////
	// Update misc
	CheckCollisionCallback(mlPlayerIndex == 0 ? "Player" : "Player2", gpBase->mpMapHandler->GetCurrentMap());
...
{
	// Determine which GUI set this player should draw to.
	// P2 without a live coop hud set draws NOTHING this frame -- falling back to
	// P1's set put P2's overlays (states, flashes, sepia wash) on P1's screen
	// whenever the coop set was transiently gone (coop toggle, split change,
	// map-change teardown).
	cGuiSet *pHudSet = gpBase->mpGameHudSet;
	if(IsPlayer2())
	{
		pHudSet = gpBase->mpMapHandler->GetCoopHudSet();
		if(pHudSet==NULL) return;
	}
	...
	mvStates[mState]->OnDraw(pHudSet, afFrameTime);

	////////////////////////
	// Run Helper message (skip the blanket broadcast for P2 — some helpers draw
	// P1-only / shared-singleton effects). P2 draws its own per-player feedback
	// explicitly instead; every helper called here routes through
	// GetDrawHudSet(), which resolves to P2's coop hud set (or nowhere).
	if(!IsPlayer2())
	{
	...
	{
		// Damage feedback: splashes + red flash.
		if(mpHudEffect) mpHudEffect->OnDraw(afFrameTime);

		// The low-health / dying red noise filter. Its Update already ran for
		// P2 (full helper update), so the alpha tracks P2's OWN health -- only
		// the draw was ever missing, which is why P2 never saw the red screen
		// P1 gets when hurt.
		if(mpHurt) mpHurt->OnDraw(afFrameTime);
	}

	////////////////////////
	// Coop: flashback sepia wash.
	// The real sepia is a post effect, and post effects never run in split-screen
	// (the composite belongs to the main viewport, which coop disables). Drawing it
	// onto this player's own hud set is the one route that reliably reaches both
	// halves -- the same route the flash, subtitles and transition fade take.
	// Drawn BEFORE DrawHud so the HUD stays legible on top of the tint.
	if(gpBase->mpMapHandler->GetCoopMode() && ImGuiDebugMenu::GetCoopFlashbackTint() &&
	   gpBase->mpMapHandler->GetCoopPostEffectsActive()==false)
	{
		gpBase->mpEffectHandler->GetSepiaColor()->DrawCoopOverlay(pHudSet);
	}
...
void cLuxPlayer::CreateWorldEntities(cLuxMap *apMap)
{
	...
	////////////////////////
	// Co-op avatar entities
	if(mpAvatar) mpAvatar->CreateWorldEntities(apMap);
}
...
void cLuxPlayer::DestroyWorldEntities(cLuxMap *apMap)
{
	...
	////////////////////////
	// Co-op avatar entities
	if(mpAvatar) mpAvatar->DestroyWorldEntities(apMap);
}
//-----------------------------------------------------------------------
...
{
	//Coop: this fires on P1 whoever opened the menu, because P1 is the only
	//registered module. The world no longer stops for a bag, so P2 opening theirs
	//must not drop P1 out of a lever or a shoved crate mid-pull.
	if(gpBase->mpMapHandler->GetCoopMode())
	{
		cLuxPlayer *pMenuOwner = NULL;

		if(asNewContainer == "Inventory")
			pMenuOwner = gpBase->mpInventory->GetActivePlayer();
		else if(asNewContainer == "Journal" && gpBase->mpJournal->GetShowOnBothPlayers()==false)
			pMenuOwner = gpBase->mpJournal->GetActivePlayer();

		if(pMenuOwner && pMenuOwner != this) return;
	}
...
cLuxPlayer* cLuxPlayer::CoopBeginActAs(cLuxPlayer *apPlayer)
{
	if(apPlayer == NULL || apPlayer == gpBase->mpPlayer) return NULL;

	cLuxPlayer *pPrev = gpBase->mpPlayer;
	gpCoopSwappedOutPlayer = pPrev;
	gpBase->mpPlayer = apPlayer;

	return pPrev;
}

void cLuxPlayer::CoopEndActAs(cLuxPlayer *apPrev)
{
	if(apPrev == NULL) return;

	gpBase->mpPlayer = apPrev;
	gpCoopSwappedOutPlayer = NULL;
}

cLuxPlayer* cLuxPlayer::CoopGetSwappedOutPlayer()
{
	return gpCoopSwappedOutPlayer;
}

//-----------------------------------------------------------------------

bool cLuxPlayer::CheckCoopCollision(iLuxEntity *apEntity, cLuxMap *apMap, bool abAlreadyColliding, cLuxPlayer **apTriggerPlayerOut)
{
	// We are already inside it, so we are the one who tripped it.
	if(abAlreadyColliding)
	{
		if(apTriggerPlayerOut) *apTriggerPlayerOut = this;
		return true;
	}

	if(ImGuiDebugMenu::GetP2EnterCallbacks()==false) return false;

	cLuxPlayer *pOther = IsPlayer2() ? gpBase->mpPlayer : gpBase->mpPlayer2;
	if(pOther == NULL || pOther == this) return false;
	if(pOther->GetCharacterBody() == NULL) return false;

	if(pOther->CheckEntityCollision(apEntity, apMap) == false) return false;

	if(apTriggerPlayerOut) *apTriggerPlayerOut = pOther;
	return true;
}
...
void cLuxPlayer::GiveDamage(float afAmount, int alStrength, eLuxDamageType aType, bool abSpinHead, bool abLethal)
{
	...
	if(mbNoclipActive) return; // Invulnerable while noclipping
...
{	
	if (mbNoclipActive)
	{
		// Movement is handled in UpdateNoclip via direct camera/body positioning.
		// Just track that the player is pressing move for other systems.
		mbPressedMove = true;
		return;
	}
...
void cLuxPlayer::DoAction(eLuxPlayerAction aAction, bool abPressed)
{
	if(mvStates[mState]->OnDoAction(aAction, abPressed))
	{
		...
		if(aAction== eLuxPlayerAction_DebugGun && abPressed)
		{
			mpGun->SetActive(!mpGun->IsActive());
		}
...
void cLuxPlayer::ReenterCurrentMoveState()
{
	mvMoveStates[mMoveState]->OnEnterState(eLuxMoveState_LastEnum);
}
...
void cLuxPlayer::AddSanity(float afX, bool abShowEffect)
{
	...
	{
		//Coop: flash the screen of the player who actually gained, not P1's.
		gpBase->mpEffectHandler->GetSanityGainFlash()->StartForPlayer(this);
	}
}
...
cLuxPlayer* cLuxPlayer::CoopGetOtherPlayer()
{
	cLuxPlayer *pP2 = gpBase->mpPlayer2;
	if(pP2 == NULL) return NULL;

	if(this != pP2) return pP2;

	//I am P2, so P1 is whichever of the acting / swapped-out pair is not me.
	cLuxPlayer *pSwapped = CoopGetSwappedOutPlayer();
	if(pSwapped && pSwapped != this) return pSwapped;
	if(gpBase->mpPlayer && gpBase->mpPlayer != this) return gpBase->mpPlayer;
...
void cLuxPlayer::SetTinderboxes(int alX)
{
	mlTinderboxes = alX;

	if(gpBase->mpMapHandler->GetCoopMode() && ImGuiDebugMenu::GetShareTinderboxes())
	{
		cLuxPlayer *pOther = CoopGetOtherPlayer();
		if(pOther) pOther->mlTinderboxes = alX;
	}
}

//-----------------------------------------------------------------------

void cLuxPlayer::AddTinderboxes(int alX)
{
	SetTinderboxes(mlTinderboxes + alX);
}

//-----------------------------------------------------------------------

void cLuxPlayer::CoopSyncSharedTinderboxes()
{
	if(gpBase==NULL || gpBase->mpMapHandler==NULL) return;
	if(gpBase->mpMapHandler->GetCoopMode()==false) return;
	if(ImGuiDebugMenu::GetShareTinderboxes()==false) return;

	cLuxPlayer *pP2 = gpBase->mpPlayer2;
	if(pP2 == NULL) return;

	cLuxPlayer *pP1 = pP2->CoopGetOtherPlayer();
	if(pP1 == NULL) return;

	int lCount = pP1->mlTinderboxes > pP2->mlTinderboxes ? pP1->mlTinderboxes : pP2->mlTinderboxes;

	pP1->mlTinderboxes = lCount;
	pP2->mlTinderboxes = lCount;
}

//-----------------------------------------------------------------------

cConfigFile* cLuxPlayer::GetPlayerCfg()
{
	if(IsPlayer2() && gpBase->mpGameCfgP2) return gpBase->mpGameCfgP2;
	return gpBase->mpGameCfg;
}

//-----------------------------------------------------------------------

tString cLuxPlayer::ResolveVoiceSound(const tString& asSound)
{
	if(IsPlayer2()==false || asSound=="") return asSound;

	std::map<tString, tString>::iterator it = m_mapVoiceSoundCache.find(asSound);
	if(it != m_mapVoiceSoundCache.end()) return it->second;

	cFileSearcher *pSearcher = gpBase->mpEngine->GetResources()->GetFileSearcher();

	tString sResolved = asSound;

	// Prefix variants first (the Justine content is codenamed "ptest"),
	// then a female-suffix variant. First existing sound wins.
	const char *vPrefixes[] = { "ptest_", "justine_" };
	for(int i=0; i<2; ++i)
	{
		tString sVariant = tString(vPrefixes[i]) + asSound;
		if(pSearcher->GetFilePath(sVariant + ".snt") != _W(""))
		{
			sResolved = sVariant;
			break;
		}
	}
	if(sResolved == asSound && pSearcher->GetFilePath(asSound + "_female.snt") != _W(""))
	{
		sResolved = asSound + "_female";
	}

	m_mapVoiceSoundCache[asSound] = sResolved;
	return sResolved;
}
...
{
	if(!mpHands) return; // P2 has no hands
...
void cLuxPlayer::SetNoclipEnabled(bool abX)
{
	mbNoclipEnabled = abX;

	// If disabling noclip permission while noclip is active, deactivate it
	if (!abX && mbNoclipActive)
	{
...
void cLuxPlayer::SetNoclipActive(bool abX)
{
	if (mbNoclipActive == abX) return;
	if (abX && !mbNoclipEnabled) return;
	...
	{
		// --- Enter noclip ---
		// Keep camera attached. Just disable physics so the body
		// can be moved freely without collision or gravity.
		mpCharBody->SetTestCollision(false);
		mpCharBody->SetGravityActive(false);
		mpCharBody->SetForceVelocity(cVector3f(0, 0, 0));
	}
	else
	{
		// --- Exit noclip ---
		// Re-enable physics. The body is already where we left it,
		// camera is still attached, nothing to reconcile.
		mpCharBody->SetForceVelocity(cVector3f(0, 0, 0));
		mpCharBody->SetTestCollision(true);
...
void cLuxPlayer::UpdateNoclip(float afTimeStep)
{
	...
	{
		SetNoclipActive(!mbNoclipActive);
	}
	mbNoclipKeyWasDown = bVDown;

	if (!mbNoclipActive) return;
	...
	// ----- Build movement vector from camera orientation -----
	...
	cVector3f vDelta = vMove * fSpeed * afTimeStep;

	// Move the character body directly. The camera is still attached
	// so it follows naturally through the normal body->camera update.
	// Use smooth=true to preserve the camera smoothing buffer.
	cVector3f vNewPos = mpCharBody->GetPosition() + vDelta;
	mpCharBody->SetPosition(vNewPos, true);

	// Keep velocity zeroed so the body doesn't accumulate drift
	mpCharBody->SetForceVelocity(cVector3f(0, 0, 0));
}

//-----------------------------------------------------------------------

cVector3f cLuxPlayer::GetInteractionForward()
{
	// Interaction ray = camera forward. Kept as a hook so a per-viewport
	// correction (e.g. differing aspect on a dual-monitor setup) has one
	// place to live if it is ever needed.
	return mpCamera->GetForward();
}

//-----------------------------------------------------------------------

bool cLuxPlayer::IsGrabbingEntity(iLuxEntity *apEntity)
{
	if(mState != eLuxPlayerState_InteractGrab) return false;
	
	// The entity we're interacting with is stored in the state's mpEntityInFocus
	// which was set before entering the grab state.
	// We can check via the DefaultBase's stored entity.
	// Simpler: if we're in grab state, we're grabbing *something*.
	// Since both players can't easily check which entity, just block grabbing
	// any entity when the other player is in grab state with the same entity.
	// For now, check if our focused entity matches.
	return (mpEntityInFocus == apEntity);
}
...
void cLuxPlayer::DrawHud(float afFrameTime)
{
	...
	// P2 draws to its own GUI set attached to P2's viewport. No set -> no draw;
	// see the note in OnDraw.
	cGuiSet *pHudSet = gpBase->mpGameHudSet;
	if(IsPlayer2())
	{
		pHudSet = gpBase->mpMapHandler->GetCoopHudSet();
	...
	if(CanDrawCrossHair())
	{
		...
		if(pCrossGfx)
		{
			...
			{
				pHudSet->DrawGfx(pCrossGfx,vFinalPos,vGfxSize,cColor(1,1));
			}
		}
	}
	...
	// Gun reticle
	//
	// Outside CanDrawCrossHair on purpose: that gate is about having something in
	// focus to interact with, and a gun has to be aimable at empty air.
	if(mpGun) mpGun->DrawCrosshair(pHudSet, vSetSize);
	...
	if(msFocusText != _W(""))
	{
		...
		for(size_t i=0; i<vRows.size(); ++i)
			pHudSet->DrawFont(vRows[i],mpFocusFont,cVector3f(400, fFocusTextY+i*24,1),22,cColor(1,mfFocusTextAlpha),	eFontAlign_Center);
	}
...
{
	tString sBodyName = mlPlayerIndex == 0 ? "Player" : "Player2";
	mpCharBody = apPhysicsWorld->CreateCharacterBody(sBodyName, mvBodySize);

	// Coop collision blockers: each player's movement must ignore their OWN
	// blocker proxy (the kinematic box cLuxMapHandler parks on their body for
	// player-vs-player collision / crouch boosting) while still colliding with
	// the other player's. Bit 14 marks P1's blockers, bit 15 marks P2's; mask
	// our own bit out. Identity for everything else -- every other body carries
	// eFlagBit_All and still matches.
	mpCharBody->SetCollideFlags(eFlagBit_All & ~(mlPlayerIndex == 0 ? eFlagBit_14 : eFlagBit_15));
