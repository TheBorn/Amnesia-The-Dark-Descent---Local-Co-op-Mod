...
public:	
	...
	/**
	 * Show or hide the model according to the Show Water Lurkers debug option.
	 *
	 * The lurker hides its mesh in two places -- on load and on every activation
	 * -- so the option has to be applied in both, and once per frame besides:
	 * toggling it mid-game calls neither of those, and without the frame check
	 * nothing would appear until the enemy happened to reactivate.
	 *
	 * mbMeshShown remembers what was last applied so the frame check is one bool
	 * compare rather than two setter calls on every lurker every frame.
	 */
