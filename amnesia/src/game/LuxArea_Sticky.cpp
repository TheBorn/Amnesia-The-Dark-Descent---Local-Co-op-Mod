...
void cLuxArea_Sticky::AttachBody(iPhysicsBody *apBody)
{
	...
	//Coop: the area finds the body geometrically and is never told WHICH player
	//shoved it in, so notify both. Each player's state only reacts if the body
	//belongs to the prop THAT player is holding, so this is a no-op for whoever
	//was not involved. Notifying P1 only meant P2 stayed stuck in InteractGrab
	//holding a body the area was slerping away -- and when both players held the
	//same body, the grab count never reached zero, so the mass/gravity snapshot
	//taken below recorded the GRAB-modified values and the prop came back out of
	//the socket permanently weightless.
	//Must stay ABOVE that snapshot: releasing every holder first is what makes
	//the recorded mass and gravity the body's true originals.
	cLuxPlayer *vPlayers[2] = { gpBase->mpPlayer, gpBase->mpPlayer2 };
	for(int i=0; i<2; ++i)
	{
		if(vPlayers[i] == NULL) continue;
		vPlayers[i]->GetCurrentStateData()->OnAttachBodyToStickyArea(apBody);
	}
