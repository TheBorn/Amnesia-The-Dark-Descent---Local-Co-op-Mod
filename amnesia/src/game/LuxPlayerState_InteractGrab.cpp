...
void cLuxPlayerState_InteractGrab::OnEnterState(eLuxPlayerState aPrevState)
{
	...
	else
	{
		...
		cVector3f vBodyInCam = cMath::MatrixMul(mtxInvCamRot, mpCurrentBody->GetLocalPosition() - pCam->GetPosition());
		
		// Compute depth first so we can reference it
		float fGrabDepth;
		if(mpGrabData->mbGrabUseDepth)
			fGrabDepth = mpGrabData->mfGrabDepth;
		else
			fGrabDepth = cMath::Vector3Dist(mvCurrentFocusPos, pCam->GetPosition()) - 0.08f;

		// Compute offset relative to the anchor point used in PostUpdate: (0, 0, -depth).
		// This ensures the object stays exactly where it is at the moment of grab.
		mvLocalBodyOffset = vBodyInCam - cVector3f(0, 0, -fGrabDepth);
	}
	...
	//Use the static grab registry so that when a second player grabs the same
	//body, we don't double-modify properties. The registry stores the TRUE
	//original values from before ANY player grabbed the body.
	mfMassSum = 0;
	...
	for(size_t i=0; i<mvBodyProperties.size(); ++i)
	{
		...
		// Check if another player is already grabbing this body
		auto it = mmapGrabbedBodies.find(pBody);
		if(it == mmapGrabbedBodies.end())
		{
			// First grab — save true original properties into registry
			cOriginalBodyProps origProps;
			origProps.mbHasGravity = pBody->GetGravity();
			origProps.mbCollideCharacter = pBody->GetCollideCharacter();
			origProps.mfMass = pBody->GetMass();
			origProps.mlGrabCount = 1;
			mmapGrabbedBodies[pBody] = origProps;

			// Get the proper mass before we modify it
			fProperMassSum += pBody->GetMass();

			// Apply grab modifications
		...
		{
			// Already grabbed by another player — just increment count, don't re-modify
			it->second.mlGrabCount++;

			// Use the original mass for player slowdown calculation
			fProperMassSum += it->second.mfMass;
		}
...
void cLuxPlayerState_InteractGrab::OnLeaveState(eLuxPlayerState aNewState)
{
	...
	///////////////////////
	//Reset body — use registry to only restore when last player releases
	for(size_t i=0; i<mvBodyProperties.size(); ++i)
	{
		iPhysicsBody *pBody = mvBodyProperties[i].mpBody;
		...
		if(it != mmapGrabbedBodies.end())
		{
			...
			if(it->second.mlGrabCount <= 0)
			{
				// Last player released — restore TRUE original properties
				pBody->SetGravity(it->second.mbHasGravity);
				pBody->SetMass(it->second.mfMass);
				// Note: CollideCharacter is restored by SetDisableCollisionUntilOutSidePlayer below

				mmapGrabbedBodies.erase(it);
			}
			// else: another player still holding — don't restore anything
		}
		else
		{
			// Fallback (shouldn't happen) — restore from per-instance saved properties
...
bool cLuxPlayerState_InteractGrab::OnDoAction(eLuxPlayerAction aAction,bool abPressed)
{
	...
	else if(aAction == eLuxPlayerAction_Attack)
	{
		...
		if(abPressed)
		{
			...
			mpCurrentBody->AddImpulse(mpPlayer->GetInteractionForward() * mpGrabData->mfGrabThrowImpulse);
...
// Static grab registry definition
std::map<iPhysicsBody*, cLuxPlayerState_InteractGrab::cOriginalBodyProps>
	cLuxPlayerState_InteractGrab::mmapGrabbedBodies;

bool cLuxPlayerState_InteractGrab::IsBodyGrabbed(iPhysicsBody *apBody)
{
	return mmapGrabbedBodies.find(apBody) != mmapGrabbedBodies.end();
}

int cLuxPlayerState_InteractGrab::GetBodyGrabCount(iPhysicsBody *apBody)
{
	auto it = mmapGrabbedBodies.find(apBody);
	if(it != mmapGrabbedBodies.end()) return it->second.mlGrabCount;
	return 0;
}
