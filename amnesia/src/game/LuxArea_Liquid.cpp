...
void cLuxArea_Liquid::DoBuoyancyOnBody(iPhysicsBody *apBody, float afSurfaceY, bool abInsideWater)
{
	...
	// Coop: either player may be the one holding this body, and it is the
	// HOLDER's state that says buoyancy must stay off (so it does not fight the
	// grab constraint). Asking only P1 meant a crate P2 dragged into water got
	// buoyancy switched on underneath P2's grab, and the prop jittered or tore
	// out of their hands.
	if(gpBase->mpPlayer2 && gpBase->mpPlayer2->GetCurrentStateData()->AllowBuoyancy(apBody)==false)
	{
		apBody->SetBuoyancyActive(false);
		return;
	}
	...
	// Coop: this is a range cull for a near-camera detail, so use whichever
	// player is closest. Culling on P1 alone left props sitting dead still on a
	// flat surface in front of a P2 standing at the water on their own.
	if(gpBase->mpPlayer2)
	{
		float fDistSqr2 = cMath::Vector3DistSqr(gpBase->mpPlayer2->GetCamera()->GetPosition(), apBody->GetLocalPosition());
		if(fDistSqr2 < fDistSqr) fDistSqr = fDistSqr2;
	}
...
void cLuxArea_Liquid::DoBuoyancyOnCharBody(iCharacterBody *apCharBody, float afSurfaceY, bool abInsideWater)
{
	...
	// Coop: P2 is a full cLuxPlayer with its own character body, in-water flag,
	// water speed multiplier and water step sound. Pick whichever player owns
	// this body -- otherwise P2 fails the test below, falls through to the ENEMY
	// branch, and its GetUserData() is blind-cast to iLuxEnemy*. Net effect was
	// that P2 never got wet at all: full dry running speed through water, dry
	// footstep sounds, and no entry splash.
	if(gpBase->mpPlayer2 && apCharBody == gpBase->mpPlayer2->GetCharacterBody())
	{
		pPlayer = gpBase->mpPlayer2;
	}
