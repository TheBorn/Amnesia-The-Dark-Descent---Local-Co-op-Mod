...
public:	
	...
	/////////////////////////////////
	// Static grab registry — tracks how many players hold each body
	// so properties are only modified on first grab / restored on last release.
	struct cOriginalBodyProps
	{
		bool mbHasGravity;
		bool mbCollideCharacter;
		float mfMass;
		int mlGrabCount;  // how many players currently grabbing this body
	};
	static std::map<iPhysicsBody*, cOriginalBodyProps> mmapGrabbedBodies;

	static bool IsBodyGrabbed(iPhysicsBody *apBody);
	static int  GetBodyGrabCount(iPhysicsBody *apBody);
