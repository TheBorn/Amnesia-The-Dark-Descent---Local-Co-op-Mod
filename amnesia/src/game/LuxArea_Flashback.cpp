...
void cLuxArea_Flashback::OnUpdate(float afTimeStep)
{
	...
	//////////////////////////
	// Check collision
	bool bTrigger = CollidesWithPlayer();
	// Coop: with Shared Flashback Visions, P2 entering the area triggers it too.
	if(!bTrigger && gpBase->mpMapHandler->GetCoopMode() && gpBase->mpPlayer2 && ImGuiDebugMenu::GetSharedFlashbackVisions())
	{
		cLuxPlayer *pOrigFB = gpBase->mpPlayer;
		gpBase->mpPlayer = gpBase->mpPlayer2;
		bTrigger = CollidesWithPlayer();
		gpBase->mpPlayer = pOrigFB;
	}
	if(bTrigger)
	{
		SetActive(false);
		...
		// Coop: deliberately do NOT start P2's helper as well. A flashback has no
		// per-player visuals -- cLuxPlayerFlashback::OnDraw is empty and everything
		// it produces (flash, sepia, radial blur, the voice queue) lives on the
		// single global cLuxEffectHandler. Running a second helper therefore
		// enqueued every voice line twice, fired msCallback twice, and counted the
		// completion amount twice. P1's helper drives the effect; P2 sees it because
		// cLuxEffect_Flash::OnDraw and cLuxEffect_PlayVoice::OnDraw now mirror onto
		// the coop hud set. The P2 collision test above is what makes the vision
		// "shared" -- either player walking in triggers it.
	}
}
