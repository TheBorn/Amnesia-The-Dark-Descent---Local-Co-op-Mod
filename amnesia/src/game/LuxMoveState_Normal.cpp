...
cLuxMoveState_Normal::cLuxMoveState_Normal(cLuxPlayer *apPlayer) : iLuxMoveState(apPlayer)
{
	...
	////////////////////////////
	// Sounds
	// Coop: these are the player's own effort/pain noises, so they come from the
	// player's own config -- P2 reads Justine's (ptest) game.cfg when installed.
	// The playback sites additionally run them through ResolveVoiceSound, which
	// picks up a ptest_/justine_/_female variant of the sound even when both
	// players share one config.
	cConfigFile *pVoiceCfg = mpPlayer->GetPlayerCfg();
	msJumpSound = pVoiceCfg->GetString("Player_Movement_Normal","JumpSound","player_jump");
	msCrouchSound = pVoiceCfg->GetString("Player_Movement_Normal","CrouchSound","player_crouch");
	msStandSound = pVoiceCfg->GetString("Player_Movement_Normal","StandSound","player_stand");
	...
	msFallDamageSound_Min = pVoiceCfg->GetString("Player_General","FallDamageSound_Min","");
	...
	msFallDamageSound_Med = pVoiceCfg->GetString("Player_General","FallDamageSound_Med","");
	...
	msFallDamageSound_Max = pVoiceCfg->GetString("Player_General","FallDamageSound_Max","");
}
...
void cLuxMoveState_Normal::Jump()
{
	...
	gpBase->mpHelpFuncs->PlayGuiSoundData(mpPlayer->ResolveVoiceSound(msJumpSound), eSoundEntryType_World);
...
void cLuxMoveState_Normal::SetCrouch(bool abActive)
{
	...
	if(abActive)
	{
		gpBase->mpHelpFuncs->PlayGuiSoundData(mpPlayer->ResolveVoiceSound(msCrouchSound), eSoundEntryType_World);
	...
	else
	{
		...
		if(bFits)
		{
			gpBase->mpHelpFuncs->PlayGuiSoundData(mpPlayer->ResolveVoiceSound(msStandSound), eSoundEntryType_World);
...
void cLuxMoveState_Normal::FallDamage(float afYSpeed)
{
	...
	if(fDamage >0)
	{
		...
		gpBase->mpHelpFuncs->PlayGuiSoundData(mpPlayer->ResolveVoiceSound(sSound), eSoundEntryType_World);
	}
}
