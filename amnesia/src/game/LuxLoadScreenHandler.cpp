...
void cLuxLoadScreenHandler::OnEnterContainer(const tString& asOldContainer)
{
	...
	//////////////////////////////////////////////////////////////
	// Co-op: this viewport is created at startup, so it comes BEFORE the split
	// viewports in cScene::mlstViewports and they render straight over it --
	// P1 sat on the black hint screen while P2 watched the live world.
	//
	// cLuxMapHandler::OnLeaveContainer("LoadScreen") hides them now, but assert
	// it here too: that function bails out early whenever either co-op camera is
	// momentarily NULL, which is exactly the state a map change leaves them in.
	if(gpBase->mpMapHandler)
	{
		cViewport *pCoopP1 = gpBase->mpMapHandler->GetCoopP1Viewport();
		cViewport *pCoopP2 = gpBase->mpMapHandler->GetCoopViewport();

		if(pCoopP1){ pCoopP1->SetActive(false); pCoopP1->SetVisible(false); }
		if(pCoopP2){ pCoopP2->SetActive(false); pCoopP2->SetVisible(false); }

		// Dual monitor: our viewport only covers the display the engine reports,
		// so P2's monitor would just go black. Mirror the load screen set onto it
		// so P2 reads the same text P1 does. No-ops outside dual-monitor coop.
...
void cLuxLoadScreenHandler::OnLeaveContainer(const tString& asNewContainer)
{
	...
	// Take the dual-monitor mirror back down (no-op if it was never put up).
	// The split viewports are deliberately NOT restored here -- that is
	// cLuxMapHandler::OnEnterContainer's job when Default comes back, and it
	// knows the current split layout.
	if(gpBase->mpMapHandler) gpBase->mpMapHandler->RemoveDualMenuMirror(mpGuiSet);
}
