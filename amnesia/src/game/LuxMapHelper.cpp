								...
								bool *apHitPlayer)
{
	...
	for(size_t i=0; i<vBodies.size(); ++i)
	{
		...
		{
			// Coop: BOTH players' bodies count as "the player" — without this,
			// P2 fell into the enemy branch and enemy attacks (checkPlayer=true,
			// checkEnemies=false) skipped P2 entirely, making them invulnerable.
			bool bIsPlayerBody = (pBody == gpBase->mpPlayer->GetCharacterBody()->GetCurrentBody());
			if(gpBase->mpPlayer2 && gpBase->mpPlayer2->GetCharacterBody() &&
				pBody == gpBase->mpPlayer2->GetCharacterBody()->GetCurrentBody())
			{
				bIsPlayerBody = true;
			}
		...
		// Coop: damage is individual — whichever player the shape overlaps gets
		// hurt (both, if both are inside the swing).
		else if(gpBase->mpPlayer2 && gpBase->mpPlayer2->GetCharacterBody() &&
				pBody == gpBase->mpPlayer2->GetCharacterBody()->GetCurrentBody())
		{
			gpBase->mpPlayer2->GiveDamage(fDamage,alStrength, aDamageType, true, abLethalForPlayer);
			if(apHitPlayer) *apHitPlayer = true;
		}
