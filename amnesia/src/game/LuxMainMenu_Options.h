...
private:
	...
	void AddCoopOptions(cWidgetTab* apTab);
	void AddDebugOptions(cWidgetTab* apTab);

	/**
	 * Fill the co-op view combo: the two split layouts, then one entry per
	 * connected display OTHER than the one the game window is on, each showing
	 * that monitor's name and current mode. mvCoopViewModes / mvCoopViewDisplays
	 * hold the split mode and display index each row means.
	 */
	void PopulateCoopViewList();
	int  GetCoopViewSelection(int alSplitMode, int alDisplay);
	...
	cWidgetTab *mpTabCoop;
	cWidgetTab *mpTabDebug;
...
//	cWidgetCheckBox *mpChBAdaptiveVSync;
	...
	// Co-op
	cWidgetDummy    *mpDCoopOptions;
	cWidgetComboBox *mpCBCoopView;
	std::vector<int> mvCoopViewModes;
	std::vector<int> mvCoopViewDisplays;

	cWidgetCheckBox *mpChBCoopAllowModelChange;
	cWidgetComboBox *mpCBCoopModelP1;
	cWidgetComboBox *mpCBCoopModelP2;
	cWidgetLabel    *mpLCoopModelP1;
	cWidgetLabel    *mpLCoopModelP2;

	cWidgetCheckBox *mpChBCoopTeleportBoth;
	cWidgetCheckBox *mpChBCoopSharedFlashbacks;
	cWidgetCheckBox *mpChBCoopBothAtLevelDoor;
	cWidgetCheckBox *mpChBCoopSharedSanityReward;
	cWidgetCheckBox *mpChBCoopSharedLantern;
	cWidgetCheckBox *mpChBCoopSharedOil;
	cWidgetCheckBox *mpChBCoopSharedLaudanum;
	cWidgetCheckBox *mpChBCoopSharedSanityPotion;
	cWidgetCheckBox *mpChBCoopSharedTinderboxes;
	cWidgetCheckBox *mpChBCoopP2Callbacks;
	cWidgetCheckBox *mpChBCoopFullItemShare;

	// Debug
	cWidgetCheckBox *mpChBDebugConsole;
	cWidgetCheckBox *mpChBDebugMenu;
	cWidgetCheckBox *mpChBDebugVerboseLog;
	 ...
	 * One-time warnings.
	 *
	 * Both fire on the FIRST enable only, and the acknowledgement is remembered
	 * in main_settings.cfg. The console and the debug menu share one warning --
	 * whichever is switched on first shows it; verbose logging shows none, since
	 * it only makes the game slow.
	 */
	bool DebugFeature_OnCheck(iWidget* apWidget, const cGuiMessageData& aData);
	kGuiCallbackDeclarationEnd(DebugFeature_OnCheck);

	bool ClickedDebugWarning(iWidget* apWidget, const cGuiMessageData& aData);
	kGuiCallbackDeclarationEnd(ClickedDebugWarning);

	bool CoopModelChange_OnCheck(iWidget* apWidget, const cGuiMessageData& aData);
	kGuiCallbackDeclarationEnd(CoopModelChange_OnCheck);

	bool ClickedCoopModelWarning(iWidget* apWidget, const cGuiMessageData& aData);
	kGuiCallbackDeclarationEnd(ClickedCoopModelWarning);

	/** Grey the two model combos out while the experimental gate is off. */
	void UpdateCoopModelEnabled();
};
