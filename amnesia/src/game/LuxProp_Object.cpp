...
void cLuxProp_Object_BodyCallback::OnBodyCollide(iPhysicsBody *apBody, iPhysicsBody *apCollideBody, cPhysicsContactData* apContactData)
{
	...
	if(apCollideBody->IsCharacter())
	{
		...
		//////////////////////////
		// Monster manipulation: props hurt monsters by their weight.
		//
		// The branch below is stock code that never does anything on stock data:
		// props ship with mfHitDamageAmount 0, and mlHitDamageStrength under the
		// enemy's mlToughness, which iLuxEnemy::GiveDamage discards outright.
		float fDamage = mpObject->mfHitDamageAmount;
		int lStrength = mpObject->mlHitDamageStrength;
		float fMinSpeed = mpObject->mfMinHitDamageSpeed;
		bool bByWeight = ::ImGuiDebugMenu::GetAllowKillingMonsters();

		if(bByWeight)
		{
			//Enough that a shove or a settling prop does not count, low enough that
			//any real throw does.
			fMinSpeed = 2.0f;

			//Toughness is a WEAPON tier -- below mlToughness-1 the hit is thrown
			//away and at exactly mlToughness-1 it is halved. A crate to the head is
			//not a weapon tier, so clear every tier and let mass decide instead.
			lStrength = 100;

			fDamage = ::ImGuiDebugMenu::GetOneHitPropKill()
						? 1000000.0f
						: apBody->GetMass() * ::ImGuiDebugMenu::GetPropDamagePerKg();
		}
		...
		if(fSpeed >= fMinSpeed)
		{
			...
			if(apCollideBody->GetCharacterBody()->GetUserData())
			{
				...
				//TakePropHit rate limits per ENEMY. This callback runs once per
				//contact per frame, so a barrel leaning on a monster would land a
				//dozen hits a second and "ten hits to kill" would mean one throw.
				if(bByWeight)
					pEnemy->TakePropHit(fDamage, lStrength);
				else
					pEnemy->GiveDamage(fDamage, lStrength);
			}
		}
	}
}
