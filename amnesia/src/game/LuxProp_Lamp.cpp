...
bool cLuxProp_Lamp::OnInteract(iPhysicsBody *apBody, const cVector3f &avPos)
{
	...
	else
	{
		...
		//
		// Free Use Candles skips this AND the decrement below -- one without the
		// other either still blocks you at zero or still eats your stock.
		// gpBase->mpPlayer is whoever is acting, so this covers both players.
		const bool bFreeCandles = ::ImGuiDebugMenu::GetFreeUseCandles();

		if(bFreeCandles==false && gpBase->mpPlayer->GetTinderboxes()<=0)
		{
		...
		////////////////////
		// Negate tinderboxes
		if(bFreeCandles==false) gpBase->mpPlayer->AddTinderboxes(-1);
