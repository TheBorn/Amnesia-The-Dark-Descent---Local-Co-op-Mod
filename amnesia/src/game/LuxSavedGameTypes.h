...
public:
	...
	// Coop: Player 2's own item list + equipped hand item (index into
	// mvItemsP2). Old saves simply have these empty / -1.
	int mlEquippedHandItemP2;
	cContainerVec<cLuxInventory_Item_SaveData> mvItemsP2;
};
...
public:
	...
	//////////////////////
	// Coop: split-screen configuration + Player 2's full player state.
	// Old saves lack these fields — the ctor defaults mean "coop off".
	bool mbCoopActive;
	//Whether the story declared itself co-op-aware. Saved because the global
	//script's OnGameStart, where it is normally declared, does not run on load.
	bool mbCoopStorySupport;
	//Coop Options -- story-scriptable (SetCoopAllow*); saved for the same
	//reason as mbCoopStorySupport. Old saves get the ctor defaults.
	...
	int mlCoopSplitMode;
	int mlCoopDualMonitor;
	bool mbHasPlayer2;
	cLuxPlayer_SaveData mPlayer2;
