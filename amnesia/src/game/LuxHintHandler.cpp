...
void cLuxHintHandler::OnDraw(float afFrameTime)
{
	...
	// Coop: mpGameHudSet lives only on P1's viewport in split-screen, so P2 never
	// saw a single hint. That is not cosmetic -- "you have no lantern" is a HINT,
	// so every one of P2's lantern refusals looked like the button doing nothing
	// at all, with no clue as to why. Mirror hints onto P2's hud set.
	if(gpBase->mpMapHandler->GetCoopMode())
	{
		cGuiSet *pCoopSet = gpBase->mpMapHandler->GetCoopHudSet();
		if(pCoopSet) DrawHintText(afFrameTime, pCoopSet);
	}
}
