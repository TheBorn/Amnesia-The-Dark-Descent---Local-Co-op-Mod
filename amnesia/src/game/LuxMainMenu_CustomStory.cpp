...
void cLuxMainMenu_CustomStory::CreateGui()
{
	...
	vLabels.push_back(kTranslateOr("CustomStory","StartCoop", _W("Start Co-op")));
	...
	pButton->AddCallback(eGuiMessage_ButtonPressed,this, kGuiCallback(PressStartCoop));
	...
	//Back
	pButton  = mpGuiSet->CreateWidgetButton(vPos,cVector2f(fButtonWidth,fButtonHeight),vLabels[4],mpWindow);
...
void cLuxMainMenu_CustomStory::BeginStory(bool abCoop)
{
	//////////////////////////////////////////////////////
...
bool cLuxMainMenu_CustomStory::PressStartCoop(iWidget* apWidget, const cGuiMessageData& aData)
{
	//////////////////////////////////////////////////////
	// A story that declared SupportsCoop in its settings file was built for two
	// players and needs no warning. Every other story is about to have co-op
	// forced onto it, and that is worth saying EVERY time -- it is a fact about
	// the story in front of you, not a preference you dismiss once.
	if(gpBase->mpCustomStory && gpBase->mpCustomStory->mbSupportsCoop)
	{
		BeginStory(true);
		return true;
	}
	...
	cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(
		kTranslateOr("CustomStory","StoryCoopWarningLabel", _W("Forced co-op")),
		kTranslateOr("CustomStory","StoryCoopWarningMessage",
			_W("You are about to enter a custom story which was not designed for co-op! This will force co-op on a single player based story, things were not meant to be this way in this story, there may be bugs, unexpected behaviour or soft locks. Player 2 can be teleported to player 1 with F2 and player 1 can teleport to player 2 with F1. Pressing both joysticks buttons down on the controller will teleport player 2 to player 1. Noclip can be used by player 1 by pressing V at any time but player 2 cannot noclip. For now these debug teleports and noclips are on by default to allow players to get out of potential soft locks, in the future they might need separate toggle in the debug sections. These features are off by default in co-op supported custom stories.")),
		kTranslate("MainMenu","OK"), kTranslate("Global","Cancel"),
		this, kGuiCallback(ClickedStoryCoopWarning), 760);
...
kGuiCallbackDeclaredFuncEnd(cLuxMainMenu_CustomStory, PressStartCoop);

//-----------------------------------------------------------------------

bool cLuxMainMenu_CustomStory::ClickedStoryCoopWarning(iWidget* apWidget, const cGuiMessageData& aData)
{
...
kGuiCallbackDeclaredFuncEnd(cLuxMainMenu_CustomStory, ClickedStoryCoopWarning);
...
void cLuxMainMenu_CustomStoryList::AddStoriesFromDir(const tWString& asPath, bool abTryOneLevelDeeper)
{
	//A missing folder is normal, not an error -- no workshop subscriptions, or no
	//user directory. FindFoldersInDir simply returns nothing for one.
	tWStringList lstStoryDirs;
	cPlatform::FindFoldersInDir(lstStoryDirs, asPath, false);

	//////////////////////////////////////////////////////////////////////
	// Try to create a story using every dir under the given dir
	...
	{
		const tWString sStoryPath = cString::AddSlashAtEndW(asPath + *it);
		...
		if(abTryOneLevelDeeper==false) continue;

		//The item folder itself is not a story, so look inside it once. Stops at the
		//first hit: a workshop item is one story, and carrying on would list a
		//story's own subfolders if one of them happened to validate.
		tWStringList lstInnerDirs;
		cPlatform::FindFoldersInDir(lstInnerDirs, sStoryPath, false);

		tWStringListIt innerIt = lstInnerDirs.begin();
		for(;innerIt!=lstInnerDirs.end();++innerIt)
		{
			cLuxCustomStorySettings* pInnerStory = hplNew(cLuxCustomStorySettings,());

			if(pInnerStory->CreateFromPath(cString::AddSlashAtEndW(sStoryPath + *innerIt)))
			{
				cWidgetItem* pItem = mpLBStories->AddItem(pInnerStory->msName);
				pItem->SetUserData(pInnerStory);
				break;
			}

			hplDelete(pInnerStory);
		}
	}
}
...
{
	AddStoriesFromDir(cString::AddSlashAtEndW(gpBase->msUserResourceDir +
											 cString::To16Char(gpBase->msCustomStoryPath)));
}
#endif
