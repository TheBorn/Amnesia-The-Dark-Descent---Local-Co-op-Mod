...
public:	
	...
	//Read-only, for anything that has to know whether this door actually has a
	//sound before relying on SetLocked's abEffects to play one. Many .ent files
	//leave the first two empty; the third is the rattle a locked door makes when
	//you try it, and is far more commonly set.
	const tString& GetLockOnSound(){ return msLockOnSound;}
	const tString& GetLockOffSound(){ return msLockOffSound;}
	const tString& GetInteractLockedSound(){ return msInteractLockedSound;}
