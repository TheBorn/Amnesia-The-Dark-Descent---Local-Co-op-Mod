...
protected:
	...
	/**
	 * Is the given player's character body inside this entity's bodies?
	 *
	 * The no-argument version tests P1, which is what every caller wanted
	 * before co-op existed. Anything that models a WORLD VOLUME -- slime,
	 * insanity areas -- must test both players instead, or P2 walks through
	 * it with no effect at all.
	 */
	bool CollidesWithPlayer();
	bool CollidesWithPlayer(cLuxPlayer *apPlayer);
	...
	float mfLookAtCount;
	bool mbIsLookedAt;
	// Coop: which player tripped the look-at ENTER edge, so the matching LEAVE
	// edge is delivered to the same one. Runtime only -- not serialised.
	cLuxPlayer *mpCoopLookAtPlayer;
