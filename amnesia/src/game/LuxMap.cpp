...
{
	//Debug view is a second, independent reason to walk this list. Without it
	//the areas drew nothing at all, because this early-out fires long before
	//any entity is asked to draw itself.
	const bool bDebugView = ::ImGuiDebugMenu::GetDebugView();

	if(gpBase->mpDebugHandler->GetShowEntityInfo()==false && bDebugView==false) return;
	...
	for(; entityIt != mlstEntities.end(); ++entityIt)
	{
		...
		//An area switched off by script is exactly the thing you turn debug
		//view on to find, so it is drawn regardless of active state. Everything
		//else keeps the old rule.
		const bool bAlwaysDraw = bDebugView && pEntity->GetEntityType() == eLuxEntityType_Area;

		if(pEntity->IsActive() || bAlwaysDraw)
...
void cLuxMap::PlacePlayerAtStartPos(const tString& asPosName)
{
	...
	if(pNode)
	{
		...
		// Coop: Player 2 spawns at the same start position (offset a step to
		// the side) — level transitions, checkpoints and respawns alike.
		if(gpBase->mpMapHandler->GetCoopMode() && gpBase->mpPlayer2)
		{
			gpBase->mpMapHandler->PlacePlayer2NearPlayer1();
		}
	}
}
...
void cLuxMap::LoadCheckPoint()
{
	...
	if(gpBase->mpPlayer2) gpBase->mpPlayer2->ClearTerrorEnemies();

	//////////////////////////////
	// Effects
    //Coop: map enter -- both screens.
    gpBase->mpEffectHandler->GetFade()->FadeInGlobal(0.2f);
...
void cLuxMap::AddTimer(const tString& asName, float afTime, const tString& asFunction)
{
	...
	//Coop: remember who is acting right now, so the payload lands on them when
	//the timer fires. gpBase->mpPlayer is already the triggering player here --
	//interact, use-item, look-at and trigger volumes all point it at them.
	pTimer->mlCoopActingPlayer = gpBase->mpPlayer ? gpBase->mpPlayer->GetPlayerIndex() : 0;
...
void cLuxMap::UpdateTimers(float afTimeStep)
{
	...
	for(tLuxEventTimerListIt it= mlstTimers.begin(); it != mlstTimers.end(); )
	{
		...
		{
			//Coop: put the player who set this timer back as the acting player, so
			//the payload -- sanity, messages, fades, forced look-ats -- lands on
			//them and not on whoever happens to be P1. Falls back to P1 when P2 is
			//gone (coop switched off, or the timer outlived a save/load).
			cLuxPlayer *pActing = (pTimer->mlCoopActingPlayer == 1) ? gpBase->mpPlayer2 : NULL;
			cLuxPlayer *pPrevActing = cLuxPlayer::CoopBeginActAs(pActing);
