...
{
	//Deliberately not remembering a state here: mbMeshShown starts false and
	//UpdateMeshVisibility is what decides, so a map loaded with the option
	//already on shows the model without waiting for a change.
	mbMeshShown = false;
...
void cLuxEnemy_WaterLurker::UpdateMeshVisibility()
{
	if(mpMeshEntity==NULL) return;

	const bool bShow = ::ImGuiDebugMenu::GetShowWaterLurkers();

	mpMeshEntity->SetActive(bShow);
	mpMeshEntity->SetVisible(bShow);
...
{
	//Cheap enough to check every frame, and it is the only thing that makes the
	//option take effect on lurkers already swimming about.
	if(mbMeshShown != ::ImGuiDebugMenu::GetShowWaterLurkers())
		UpdateMeshVisibility();
}	
...
{
	//Normally forces the mesh invisible -- this is the second place that does
	//it, and the one that would undo the debug option every time the enemy
	//activated.
	UpdateMeshVisibility();
}
