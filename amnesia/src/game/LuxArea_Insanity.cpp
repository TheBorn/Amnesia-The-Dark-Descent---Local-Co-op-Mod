...
void cLuxArea_Insanity::OnUpdate(float afTimeStep)
{
	...
	//////////////////////////
	// Check collision
	// Co-op: either player standing in the area trips it. CollidesWithPlayer()
	// only ever tested P1, so P2 walked through insanity areas without ever
	// setting one off. cLuxInsanityHandler is global, so it stays one event
	// per trigger no matter who walked in.
	bool bAnyPlayerInside = CollidesWithPlayer(gpBase->mpPlayer) ||
							(gpBase->mpPlayer2 != NULL && CollidesWithPlayer(gpBase->mpPlayer2));
