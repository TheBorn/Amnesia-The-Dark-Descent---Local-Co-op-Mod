...
void cLuxInstanityEvent_SoundStream::OnDraw(float afFrameTime)
{
	...
	{
		//////////////////////////////////////////////////////////////////////
		// Both halves, in co-op.
		//
		// mpWhiteGfx is MODULATIVE and full virtual size, so at full fade alpha it
		// multiplies everything already on that set down to mFadeColor -- black, for
		// the usual event definitions. And mpGameHudSet is attached to Player 1's
		// viewport only; Player 2 has its own set.
		//
		// cLuxArea_Insanity fires when EITHER player walks in, but the insanity
		// handler is a single global, so Player 2 tripping an insanity area blacked
		// out Player 1's half of the screen while Player 2 saw nothing at all. Every
		// other full-screen effect in the game was already given this treatment --
		// the transition fade, the flash, subtitles, hints, the sepia wash. This one
		// was missed.
		cGuiSet *pSets[2] = { gpBase->mpGameHudSet, NULL };
		...
		for(int i=0; i<2; ++i)
		{
			...
			if(mpFadeImage)
				pSets[i]->DrawGfx(mpFadeImage, gpBase->mvHudVirtualStartPos, gpBase->mvHudVirtualSize, mFadeColor * mfFadeAlpha);
			else
				pSets[i]->DrawGfx(mpWhiteGfx, gpBase->mvHudVirtualStartPos, gpBase->mvHudVirtualSize, mFadeColor*mfFadeAlpha + cColor(1,1)*(1-mfFadeAlpha));
		}
	}
}
