...
public:
	...
	void SetTargetHands(cLuxPlayerHands *apPlayerHands){ mpPlayerHands = apPlayerHands; }
...
public:	
	...
	/**
	 * Coop: per-viewport visibility, called right before each viewport
	 * renders. The owner's view shows the hands / lantern model (following
	 * the game's own shown/hidden intent); the other player's view hides
	 * the model, flame billboards and particles — but the lantern LIGHT is
	 * never touched, so its glow shines for everyone.
	 */
	void SetVisualsVisibleForCamera(bool abOwnView);

	const tString& GetCurrentAnimation(){ return msCurrentAnim; }
	cMeshEntity *GetHandsEntity(){ return mpHandsEntity;}
	cLuxPlayer *GetPlayer(){ return mpPlayer; }
...
private:
	...
	// Static pointer to the shared "PlayerHands" entity loader (registered once by P1).
	// P2 temporarily redirects this before creating its hand entity.
	static cLuxPlayerHandsLoader *spSharedHandsLoader;
	static cLuxHandObjectLoader *spSharedHandObjectLoader;
