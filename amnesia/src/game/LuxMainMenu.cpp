...
//	gpBase->SetDrawOnLiveCursor(false);
	...
	////////////////////////////
	//Dual-monitor coop: mirror the pause menu GUI onto P2's monitor so both
	//players see the game is paused. (No-op at the boot main menu — coop is
	//only ever active with a map loaded.)
...
void cLuxMainMenu::OnDraw(float afFrameTime)
{
	...
	if(mpScreenGfx)
	{
		...
		{
			// Dual-monitor coop: the captured/blurred snapshot is P1's view,
			// and this GUI set is mirrored onto P2's monitor — a snapshot
			// background would show P1's view on BOTH screens. Draw a
			// translucent dim instead; each monitor's own (frozen) world
			// viewport keeps rendering beneath it, so both players see the
			// pause menu over their own view.
			mpGuiSet->DrawGfx(mpBlackFade, cVector3f(0,0,0.2f), mvScreenSize, cColor(1, 0.6f*(1.0f-mfMenuFadeAlpha)));
		}
		else
		{
...
void cLuxMainMenu::RenderBlurTexture()
{
	...
	// In co-op the live framebuffer is a split-screen composite (two viewports
	// plus the black letterbox bars), so a raw CopyFrameBufferToTexure grabs a
	// misaligned/partial image and the pause background looks broken. Re-render
	// a clean full-screen composite first — the same coop-aware path the
	// inventory/journal backgrounds use (clears to black, draws both viewports
	// and the HUD). In dual-monitor mode this captures P1's full-screen view.
	if(gpBase->mpMapHandler->GetCoopMode())
	{
...
bool cLuxMainMenu::ClickedStartGamePopup(iWidget* apWidget, const cGuiMessageData& aData)
{
	...
	if(bStartGame)
	{
		//The fallback path, taken when the mode window is not offered at all.
		//No mode was chosen, so this is a plain single-player start.
		gpBase->mbCoopWanted = false;
