...
bool cLuxCustomStorySettings::CreateFromPath(const tWString& asPath)
{
	...
	if(bValid)
	{
		...
		//Co-op support, declared at the menu rather than discovered in-script.
...
bool cLuxBase::Init(const tString &asCommandline)
{
	...
	cImGuiConsole::Init();
	Lux_RegisterConsoleCommands();
	ImGuiDebugMenu::SetCoopCallbacks(
       []() -> bool { return gpBase->mpMapHandler->GetCoopMode(); },
        [](bool abX) { gpBase->mpMapHandler->SetCoopMode(abX); }
    );
	ImGuiDebugMenu::SetSplitModeCallbacks(
       []() -> int { return gpBase->mpMapHandler->GetSplitScreenMode(); },
        [](int alMode) { gpBase->mpMapHandler->SetSplitScreenMode(alMode); }
    );
	ImGuiDebugMenu::SetDualMonitorCallbacks(
	...
	// Derived, never toggled -- the story decides.
	ImGuiDebugMenu::SetForcedCoopStateCallback(
       []() -> int {
            if(gpBase==NULL || gpBase->mpMapHandler==NULL) return 0;
            if(gpBase->mpMapHandler->GetCoopMode()==false) return 0;
            return gpBase->mpMapHandler->GetCoopStorySupport() ? 2 : 1;
        }
    );
	// Coop Options bridge: the story-owned values (map handler) exposed to the
	// debug menu's Coop Options section and to the native-story routing inside
	// the ImGuiDebugMenu getters. Bools travel as 0/1.
	ImGuiDebugMenu::SetCoopOptionCallbacks(
       [](int alOpt) -> float {
            if(gpBase==NULL || gpBase->mpMapHandler==NULL) return 0.0f;
            cLuxMapHandler *pMH = gpBase->mpMapHandler;
            switch(alOpt) {
            case ImGuiDebugMenu::eImGuiCoopOption_CrouchBoost:      return pMH->GetCoopOptCrouchBoost() ? 1.0f : 0.0f;
            case ImGuiDebugMenu::eImGuiCoopOption_PlayerCollision:  return pMH->GetCoopOptPlayerCollision() ? 1.0f : 0.0f;
            case ImGuiDebugMenu::eImGuiCoopOption_MonsterKilling:   return pMH->GetCoopOptMonsterKilling() ? 1.0f : 0.0f;
            case ImGuiDebugMenu::eImGuiCoopOption_MonsterDamageMul: return pMH->GetCoopOptMonsterDamageMul();
            case ImGuiDebugMenu::eImGuiCoopOption_GlobalLantern:    return pMH->GetCoopOptGlobalLantern() ? 1.0f : 0.0f;
            }
            return 0.0f;
        },
       [](int alOpt, float afVal) {
            if(gpBase==NULL || gpBase->mpMapHandler==NULL) return;
            cLuxMapHandler *pMH = gpBase->mpMapHandler;
            switch(alOpt) {
            case ImGuiDebugMenu::eImGuiCoopOption_CrouchBoost:      pMH->SetCoopOptCrouchBoost(afVal!=0.0f); break;
            case ImGuiDebugMenu::eImGuiCoopOption_PlayerCollision:  pMH->SetCoopOptPlayerCollision(afVal!=0.0f); break;
            case ImGuiDebugMenu::eImGuiCoopOption_MonsterKilling:   pMH->SetCoopOptMonsterKilling(afVal!=0.0f); break;
            case ImGuiDebugMenu::eImGuiCoopOption_MonsterDamageMul: pMH->SetCoopOptMonsterDamageMul(afVal); break;
            case ImGuiDebugMenu::eImGuiCoopOption_GlobalLantern:    pMH->SetCoopOptGlobalLantern(afVal!=0.0f); break;
            }
        }
    );

	/////////////////////////////
	// Push the saved Debug and Coop settings into the systems that read them.
	// Has to happen here rather than in LoadMainConfig: that runs before the
	// engine exists, and these reach the map handler and the ImGui gates.
	mpConfigHandler->ApplyDebugAndCoopSettings();
...
bool cLuxBase::StartGame(const tString& asFile, const tString& asFolder, const tString& asStartPos)
{
	...
	//Co-op.
	//
	//Before the map loads, not after: with no current map SetCoopMode only
	//raises the flag and the viewports and Player 2 are built as part of the
	//first SetCurrentMap, which is the same path a loaded save takes.
	//
	//And after the Reset broadcast above, which clears both of these -- setting
	//them any earlier in the start sequence would simply be undone.
	mpMapHandler->SetCoopMode(mbCoopWanted);

	//What the custom story declared about itself in custom_story_settings.cfg.
	//The main game declares nothing and so is never a native co-op story.
	//
	//The settings file is the ONLY way to declare it. There used to be a script
	//function as well, which was redundant and worse: the menu has to know
	//before the story is entered -- that is what decides whether the
...
bool cLuxBase::InitMainConfig()
{
	...
	////////////////////////////////////
	// Coop: try to load the Justine (ptest) game config for Player 2's
	// voice / lantern identity. Justine ships with the game — when its
	// config is found, P2 uses its player definitions; otherwise P2 simply
	// mirrors P1. Read the ptest init file for the real config path instead
	// of guessing the name.
	{
#ifdef USERDIR_RESOURCES
		cConfigFile *pPTestInit = hplNew( cConfigFile, (_W("config/ptest_main_init.cfg"), msUserResourceDir) );
#else
		cConfigFile *pPTestInit = hplNew( cConfigFile, (_W("config/ptest_main_init.cfg")) );
#endif
		if(pPTestInit->Load())
		{
			tWString sPTestGameCfg = pPTestInit->GetStringW("ConfigFiles", "Game", _W(""));
			if(sPTestGameCfg != _W(""))
			{
#ifdef USERDIR_RESOURCES
				mpGameCfgP2 = hplNew( cConfigFile, (sPTestGameCfg, msUserResourceDir) );
#else
				mpGameCfgP2 = hplNew( cConfigFile, (sPTestGameCfg) );
#endif
				if(mpGameCfgP2->Load()==false)
				{
					hplDelete(mpGameCfgP2);
					mpGameCfgP2 = NULL;
				}
			}
		}
		hplDelete(pPTestInit);

		if(mpGameCfgP2)
			Log(" Loaded Justine (ptest) game config — Player 2 gets the Justine identity\n");
		else
			Log(" No Justine (ptest) game config found — Player 2 uses the default player config\n");
	}
...
void cLuxBase::ExitConfig()
{
	...
	if(mpUserKeyConfig) hplDelete(mpUserKeyConfig);
	hplDelete(mpGameCfg);
	if(mpGameCfgP2) hplDelete(mpGameCfgP2);
	hplDelete(mpMenuCfg);
...
bool cLuxBase::InitGame()
{
	...
	mpPlayer2 = NULL; // Created dynamically when coop activates
