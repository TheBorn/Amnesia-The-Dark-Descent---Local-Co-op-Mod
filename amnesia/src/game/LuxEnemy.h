...
public:
	...
	// Ragdoll pose: one body WORLD matrix per bone state, written only for a
	// corpse and empty for anything still alive. Bones with no body still take a
	// slot so this indexes 1:1 with GetBoneState().
	cContainerVec<cMatrixf> mvRagdollBodyMatrices;
...
//Possession drives the enemy through its OWN protected controls -- Attack,
//SetMoveSpeed, mvTempPos -- rather than reimplementing any of them. Same
//arrangement cLuxEnemy_ManPig already has with cLuxEnemyMover.
friend class cLuxPlayerPossess;
public:	
	...
	/**
	 * While set, GiveDamage skips the state-based resistance multiplier.
	 *
	 * Every enemy's GetDamageMul returns 0.2 once it is hunting or attacking --
	 * correct for a thrown chair, useless for the debug gun, which would need
	 * twenty hits on a monster that is actually coming at you. cLuxPlayerGun::Fire
	 * sets this around its ShapeDamage call and clears it straight after, so the
	 * shared damage path still does the sounds, the particles and the shove.
	 */
	static void SetIgnoreDamageResistance(bool abX){ mbIgnoreDamageResistance = abX;}
	static bool GetIgnoreDamageResistance(){ return mbIgnoreDamageResistance;}
	...
	/**
	 * Does this enemy actually implement eLuxEnemyState_Hurt?
	 *
	 * There is no state table to ask. The machine is a macro if/else chain inside
	 * StateEventImplement, so entering a state a subclass never declared falls
	 * through to its global handler and strands the enemy in something with no
	 * Enter, no Update and no way out.
	 *
	 * cLuxEnemy_WaterLurker is exactly that: it names eLuxEnemyState_Hurt once, in
	 * GetDamageMul, and never implements it -- and with mbUseAnimations false it has
	 * no flinch to play either. Hence FALSE by default, so a new enemy type is safe
	...
	//////////////////////
	//Possession (debug). Driven by cLuxPlayerPossess; see that class.

	bool IsPossessed(){ return mbPossessed;}

	/**
	 * Hand the body to a player, or take it back.
	 *
	 * Suspends the parts of OnUpdate that DECIDE (pathfinder, senses, the state
	 * machine's per-frame handler) and leaves the parts that EXECUTE running, so
	 * the movement, animation and attacks a player drives are the enemy's own.
	 *
	 * Also detaches everything this enemy pushes at the players -- terror, chase
	...
	/**
	 * Take a hit from a thrown physics prop, rate limited.
	 *
	 * The limiter lives here rather than on the prop because
	 * cLuxProp_Object_BodyCallback::OnBodyCollide fires once per contact per
	 * frame: a barrel resting against a monster would otherwise chew through it
	 * in half a second, and a light prop would kill on a single throw instead of
	 * the ten hits its weight is supposed to be worth.
	 */
	bool TakePropHit(float afDamage, int alStrength);

	/**
	 * Look for a physics object that is currently hitting this enemy.
	 *
	 * Asked from the enemy rather than answered by the prop's collision
	 * callback, because that callback only exists on cLuxProp_Object and only
	 * reaches a character after Newton's contact filtering and a per-body .ent
	 * attribute both agree -- too many ways for a throw to land and do nothing.
	 */
	void UpdatePropHits(float afTimeStep);

	/**
	 * Everything that has to stop happening once this enemy is dead, plus the
	 * ragdoll. Called from GiveDamage so it covers every enemy type and every
	 * route to zero health, instead of the three copy-pasted Dead state blocks.
	 */
	void OnKilled();

	/**
	 * Hand the skeleton to the physics solver. False if this model has no bodies
	 * bound to its bones, in which case there is nothing to simulate.
	 */
	bool StartRagdoll();

	/**
	 * Create a physics body per bone plus the ball joints between them, for
	 * models that ship without an authored ragdoll -- which is all of them.
	 *
	 * False if the mesh has no usable skeleton. Safe to call twice: bones that
	 * already own a body are left alone.
	 */
	bool BuildRagdollBodies();

	bool IsKilled(){ return mbKilled;}
	bool IsRagdolled(){ return mbRagdollActive;}
...
protected:
	...
	/**
	 * Coop: the player this enemy is currently focused on. Never returns NULL
	 * — with coop off (or P2 gone) it is always Player 1. All the "player"
	 * senses, distances and chase positions below go through this.
	 */
	cLuxPlayer* GetTargetPlayer();

	/**
	 * Coop: re-evaluates which player to focus on. If both players are in
	 * view at similar distances the pick is sticky (random on the first
	 * detection), but an OBVIOUSLY closer player (under ~60% of the other's
	 * distance) always wins. Runs every ~0.75s from OnUpdate.
	 */
	void UpdateTargetPlayer(float afTimeStep);

	// Instant visibility test used for retargeting (range with darkness /
	// crouch handicaps + FOV + line of sight).
	bool CanDetectPlayerNow(cLuxPlayer *apPlayer);

	// The sight range against a specific player, with the lantern / darkness /
	// crouch handicaps applied.
	float CalcPlayerSeeRange(cLuxPlayer *apPlayer);

	// Removes this enemy from the terror (heartbeat) state of BOTH players.
	void RemoveTerrorFromAllPlayers();
	...
	// Possession. Deliberately NOT serialized: a save taken mid-possession would
	...
	// Monster manipulation. mbKilled means OnKilled has run (senses, music,
	// terror and sanity all shut down); mbRagdollActive additionally means the
	// physics ragdoll engaged, which needs bone bodies in the model to exist.
	bool mbKilled;
	bool mbRagdollActive;
	float mfPropHitCooldown;
	// Pose read back from a save, consumed when the corpse is rebuilt. Empty
	// unless this enemy was loaded already dead.
	std::vector<cMatrixf> mvSavedRagdollPose;

	static bool mbIgnoreDamageResistance;
	...
	// Coop target player. Not saved — resolves to P1 on load and re-evaluates
	// within a second. Never dereferenced without validation (see
	// GetTargetPlayer), since P2 can be destroyed at any time.
	cLuxPlayer *mpTargetPlayer;
	float mfRetargetCount;
