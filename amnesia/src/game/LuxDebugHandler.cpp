...
void cLuxDebugHandler::RenderSolid(cRendererCallbackFunctions* apFunctions)
{
	...
	//////////////////////////////////////////////////////////////////
	// Show colliders -- the model editor's view, for the whole world.
	//
	// Body by body rather than the world's own RenderDebugGeometry, which draws
	// everything including the character bodies. You are stood inside yours: it
	// fills the screen and hides the very thing you turned this on to look at.
	if(::ImGuiDebugMenu::GetShowColliders())
	{
		cLuxMap *pMap = gpBase->mpMapHandler->GetCurrentMap();
		if(pMap)
		{
			iPhysicsBody *pSkipP1 = NULL;
			iPhysicsBody *pSkipP2 = NULL;

			if(gpBase->mpPlayer && gpBase->mpPlayer->GetCharacterBody())
				pSkipP1 = gpBase->mpPlayer->GetCharacterBody()->GetCurrentBody();
			if(gpBase->mpPlayer2 && gpBase->mpPlayer2->GetCharacterBody())
				pSkipP2 = gpBase->mpPlayer2->GetCharacterBody()->GetCurrentBody();

			iLowLevelGraphics *pGfx = apFunctions->GetLowLevelGfx();

			cPhysicsBodyIterator it = pMap->GetPhysicsWorld()->GetBodyIterator();
			while(it.HasNext())
			{
				...
				pBody->RenderDebugGeometry(pGfx,
					pBody->GetMass() > 0 ? cColor(0.3f,1.0f,0.4f,1) : cColor(0.55f,0.55f,0.6f,1));
			}
		}
	}
}
