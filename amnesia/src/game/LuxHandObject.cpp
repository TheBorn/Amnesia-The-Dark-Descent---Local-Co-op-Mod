...
void iLuxHandObject::CreateEntity(cLuxMap *apMap)
{
	...
	///////////////////////
	// Load the entity (use unique name for P2 to avoid collisions)
	tString sFile = "models/player/"+msName+"/"+ msModelFile;
	tString sEntityName = "PlayerHands";
	if(mpHands && mpHands->GetPlayer() && mpHands->GetPlayer()->IsPlayer2())
		sEntityName = "PlayerHands_P2_" + msName;
	apMap->GetWorld()->CreateEntity(sEntityName, cMatrixf::Identity, sFile);
...
void iLuxHandObject::SetModelVisible(bool abVisible)
{
	if(mpMeshEntity && mpMeshEntity->IsVisible() != abVisible)
	{
