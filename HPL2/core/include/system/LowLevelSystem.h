...
#define UPDATE_TIMING_ENABLED
	...
	//Where SetLogFile put it. Anything that wants to drop a file next to hpl.log --
	//a crash dump, a frame capture -- needs the directory and nothing else knows it.
	extern const tWString& GetLogFilePath();
	 ...
	 * Verbose logging.
	 *
	 * LogVerbose writes to hpl.log exactly like Log, but only while the verbose
	 * switch is on -- it is off unless the player turns it on in Options ->
	 * Debug. This is where the render and co-op instrumentation goes: per-frame
	 * pacing, present gaps, window events, screen-covering fades. Useful when
	 * chasing a bug, ruinous for performance and log size otherwise, since some
	 * of it costs a GPU sync per frame.
	 */
	extern void SetLogVerbose(bool abX);
	extern bool GetLogVerbose();
	extern void LogVerbose(const char* fmt, ...);
