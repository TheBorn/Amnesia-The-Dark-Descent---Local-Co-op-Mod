...
#define END_RENDER_PASS() \
	...
	protected:
		...
		*/
		...
		/**
		 * Companion to the above for the texture coords, which CreateQuadVertexBuffer
		 * bakes once at load time. Needed because the render scale changes which part
		 * of a buffer a full-buffer quad is supposed to sample.
		 */
		void UpdateQuadVertexUV(iVertexBuffer *apVtxBuffer, const cVector2f& avMinUV, const cVector2f& avMaxUV,
	...
	public:
		...
		inline void SetFrustumProjection(cFrustum *apFrustum) { mpRenderer->SetFrustumProjection(apFrustum); }
		inline void SetProjectionMatrix(const cMatrixf *apProjMatrix) { mpRenderer->SetProjectionMatrix(apProjMatrix); }
		...
		//Render scale. A callback that switches frame buffers and composites the
		//result back is drawing into the same sub-rect as the renderer, so it has to
		//sample the same one. Identity at scale 1.
		inline bool IsRenderScaled(){ return mpRenderer->IsRenderScaled(); }
		inline void GetScaledUVRange(const cVector2f& avTexSize, cVector2f& avMinUV, cVector2f& avMaxUV)
							{ mpRenderer->GetScaledUVRange(avTexSize, avMinUV, avMaxUV); }
