...
namespace hpl {
	...
	private:
		...
		/**
		 * Apply the debug menu's render scale to this pass.
		 *
		 * Must be re-applied after anything that rebuilds the render state mid-pass,
		 * and ALWAYS before SetupInternalBufferRenderTarget(), which derives the
		 * render target size from it.
		 */
		void ApplyRenderScale();

		/**
		 * The area of this pass's internal buffers that actually gets rasterised.
		 *
		 * Anything that has to line up with what was drawn -- the render target, a
		 * scissor rect, a clip rect -- must come from here rather than compute its
		 * own, or the two round differently and drift.
		 */
		cVector2l GetInternalBufferRenderSize(bool abIsReflection);

		/**
		 * Point the render target at the internal buffer set this pass writes to.
		 * Must be re-applied after anything that rebuilds the render state mid-pass.
		 */
		void SetupInternalBufferRenderTarget();
