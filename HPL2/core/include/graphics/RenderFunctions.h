...
namespace hpl {
	...
	public:
		...
		/**
		 * Render into the TOP-LEFT sub-rect of every internal buffer instead of
		 * filling it. (1,1) is off and every helper below becomes an identity.
		 *
		 * Top-left, not bottom-left, on purpose: the GL viewport and the source
		 * texels then share absolute framebuffer coordinates, so shaders that
		 * address their input by gl_FragCoord keep working, and mvPos can stay
		 * (0,0) so scissor rects and buffer-to-texture copies need no offset.
		 */
		void SetRenderScale(const cVector2f& avScale);
		const cVector2f& GetRenderScale() const { return mvRenderScale; }
		bool IsRenderScaled() const { return mvRenderScale.x < 1.0f || mvRenderScale.y < 1.0f; }

		cVector2l GetScaledSize(const cVector2l& avSize) const;
		cVector2f GetScaledSizeF(const cVector2f& avSize) const;

		/**
		 * Pixel UV range addressing the scaled sub-rect of a full-size buffer.
		 * Rect textures are BOTTOM-origin and the sub-rect is anchored at the TOP,
		 * so y runs [texH - scaledH, texH]. Derived from the INTEGER scaled size so
		 * it lines up exactly with the viewport the pass was rendered through.
		 */
		void GetScaledUVRange(const cVector2f& avTexSize, cVector2f& avMinUV, cVector2f& avMaxUV) const;
	...
	protected:
		...
		cVector2f mvRenderScale;		//(1,1) = render at full buffer size.
