...
namespace hpl {
	...
	{
	public:
		cGuiRenderObject() : mpGfx(NULL), mvPos(0), mvSize(0), mColor(0,0), mpCustomMaterial(NULL), mpClipRegion(NULL), mbRotated(false), mfAngle(0.0f), mvPivot(0.0f), mlAddIndex(0)
		{}
		...
		//The order this object was submitted in, counted from zero every frame. It is
		//what decides draw order among objects at the same z -- see the comparator.
		int mlAddIndex;
	};
	...
	public:
		...
		////////////////////////////////////
		// Popup
		/** afMaxTextWidth > 0 word wraps the body text -- see cGuiPopUpMessageBox. */
	...
	private:
		...
		//Submission counter for m_setRenderObjects, restarted every frame in
		//ClearRenderObjects.
		int mlRenderObjectAddCount;
