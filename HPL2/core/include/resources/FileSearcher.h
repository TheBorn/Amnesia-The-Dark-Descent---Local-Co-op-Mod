...
namespace hpl {
	...
	public:
		...
		/**
		 * Every indexed file, keyed by file name. Read-only.
		 *
		 * Exposed for console autocompletion: the searcher has already walked every
		 * mounted resource directory, so listing what the game can load is a map
		 * walk rather than a disk scan -- and it picks up custom stories and mods
		 * for free, since those are mounted through here too.
		 */
		const tFilePathMap& GetFileMap() const { return m_mapFiles; }
