...
namespace hpl {
	...
	public:
		...
		/**
		 * Whether a category/entry pair is actually present.
		 *
		 * Translate() warns to the log and hands back an empty string when it is
		 * not, which is fine for content the language file is required to carry
		 * but wrong for text that ships with a built-in default. This answers
		 * the same question quietly.
		 */
		bool EntryExists(const tString& asCat, const tString& asName);
