...
namespace hpl {
	...
	//////////////////////////////////////////////////////////////////////////
	// Per-frame gui stats for the flight recorder in cLowLevelGraphicsSDL.
	// Written here during draws/renders, recorded and reset once per present.
	int gLuxDbgFrameGuiObjects = 0;
	float gLuxDbgFrameMinQuadMul = 1.0f;
	char gLuxDbgFrameQuadSet[20] = {0};
												...
												const cGuiRenderObject& aObjectB) const
	{
		...
		//////////////////////////////////////////////////////////////////////////
		// Same z: submission order, and nothing else.
		//
		// THE MENU GUI FLICKER.
		//
		// This used to tie-break on the raw addresses of the clip region, then the
		// material, then the texture -- grouping draws to save state changes. The
		// material and texture pointers are stable for the life of the set, so those
		// two were harmless. The clip region is not: DrawAll hplNews a fresh child for
		// every clipping widget every single frame and Clear hplDeletes them all again.
		//
		// So the draw order of two overlapping elements at the same z was decided by
		// where the allocator happened to put their clip regions THIS frame. Reuse the
		// same addresses and the order holds; hand back different ones -- which any
		// allocation churn elsewhere causes -- and the pair swaps over. One frame the
		// label is on top of its background, the next frame it is behind it and gone.
		// Text and images flicker and drop out while the 3D scene sits there perfectly
		// still, because the scene never goes through this list.
		//
		// Submission order is deterministic, is the painter's order the widgets already
		// draw themselves in, and cannot vary between two frames that drew the same
		// thing. Batching still happens wherever consecutive draws share a material,
		// which in a GUI is nearly all of them -- a few extra state changes is not a
		// trade worth a visibly unstable interface.
		if(aObjectA.mlAddIndex != aObjectB.mlAddIndex)
		{
			return aObjectA.mlAddIndex < aObjectB.mlAddIndex;
		}
	...
	void cGuiSet::Render(cFrustum *apFrustum)
	{
		...
		///////////////////////////////
		//The clip regions used to be freed HERE, under a comment claiming it
		//cleared the render object set. It does not -- it hplDeletes every child
		//of mBaseClipRegion, and m_setRenderObjects survives until
		//cGui::OnPostBufferSwap calls ClearRenderObjects. Every render object
		//holds an mpClipRegion pointing into what was just freed.
		//
		//One render per frame hides it: nothing reads those pointers again before
		//they are dropped. Render the same set TWICE in a frame -- two visible
		//viewports sharing it, or a Draw/Render pair after a Render -- and the
		//second pass dereferences freed memory at SetClipArea, clipping text and
		//images to whatever the heap now holds. The 3D scene is untouched because
		//it never goes through this list, which is exactly what "the menu text and
		//logo flicker but the background map is fine" looks like.
		//
		//Freed in ClearRenderObjects instead, so the regions and the objects
		//pointing at them die together. Every caller already pairs Render with
		//ClearRenderObjects, and DrawAll rebuilds the children from scratch.
	...
	void cGuiSet::ClearRenderObjects()
	{
		...
		//Moved here from the end of Render. The render objects hold pointers into
		//these regions, so the two have to be dropped together -- see the note
		//there. mBaseClipRegion itself is a member and is never freed; only the
		//children DrawAll creates each frame are.
							...
							bool abUseCustomPivot, const cVector3f& avCustomPivot)
	{
		...
		////////////////////////////////////////////////////////////////////////
		// Black flicker hunt -- TEMPORARY
		//
		// The frame dump showed the scene rendered correctly and then scaled down to
		// about a fifteenth of its brightness, with one late element left untouched on
		// top of it. The obvious shape for that is a near-full-screen quad in a dark
		// colour, and every screen effect in the game draws through this one function.
		// So instead of naming suspects one at a time, the draw reports itself.
		//
		// FIXED: the first version read aMaterial, the ARGUMENT -- but nearly every
		// dark quad in the game (the transition fade, the death fade, the insanity
		// fade) leaves it at eGuiMaterial_LastEnum and carries its real material on
		// the gfx element, resolved four lines below. So the probe was blind to all
		// of them, and its silence exonerated nobody. Resolve the EFFECTIVE material
		// exactly the way the renderer will.
		//
		// fMul is what the quad does to what is already on screen:
		//   modulative  dst = dst * col          -> mul is the brightest channel
		//   alpha       dst = col*a + dst*(1-a)  -> mul = 1 - a*(1-brightest)
		//   additive    only ever brightens      -> ignored
		{
			const float fBright = cMath::Max(cMath::Max(aColor.r, aColor.g), aColor.b);

			iGuiMaterial *pEffMat = (aMaterial != eGuiMaterial_LastEnum) ?
										mpGui->GetMaterial(aMaterial) : apGfx->mpMaterial;

			float fMul = 1.0f;
			if(pEffMat == mpGui->GetMaterial(eGuiMaterial_Modulative))	fMul = fBright;
			else if(pEffMat == mpGui->GetMaterial(eGuiMaterial_Alpha))	fMul = 1.0f - aColor.a * (1.0f - fBright);

			if(	object.mvSize.x >= mvVirtualSize.x*0.9f &&
				object.mvSize.y >= mvVirtualSize.y*0.9f)
			{
				//Feed the flight recorder: darkest full-screen quad this frame and
				//which set drew it. Recorded every frame, dumped on F9.
				if(fMul < gLuxDbgFrameMinQuadMul)
				{
					gLuxDbgFrameMinQuadMul = fMul;
					size_t lN = msName.size() < 19 ? msName.size() : 19;
					for(size_t c=0; c<lN; ++c) gLuxDbgFrameQuadSet[c] = msName[c];
					gLuxDbgFrameQuadSet[lN] = 0;
				}

				if(fMul < 0.35f && GetLogVerbose())
				{
					static float sfLastMul = -1.0f;
					static int slLinesLogged = 0;

					//Only on a change, and capped: a dimmer parked at a constant value is
					//somebody's menu background, not a flicker, and it would bury the log.
					if(cMath::Abs(fMul - sfLastMul) > 0.02f && slLinesLogged < 200)
					{
						sfLastMul = fMul;
						++slLinesLogged;
						LogVerbose("=== DARK QUAD at %lu ms: set '%s' mul=%.3f mat=%d col=(%.2f,%.2f,%.2f,%.2f)"
							" size=(%.0f,%.0f) virt=(%.0f,%.0f) z=%.2f\n",
							cPlatform::GetApplicationTime(), msName.c_str(), fMul, (int)aMaterial,
							aColor.r, aColor.g, aColor.b, aColor.a,
							object.mvSize.x, object.mvSize.y,
							mvVirtualSize.x, mvVirtualSize.y, vAbsPos.z);
					}
				}
			}
		}
		...
		object.mlAddIndex = mlRenderObjectAddCount++;
...
#define kLogRender (false)
	...
	void cGuiSet::RenderClipRegion()
	{
		...
		//Flight recorder: objects actually rendered this frame. A set rendered by
		//two viewports counts twice -- deliberately, that doubling is diagnostic.
		gLuxDbgFrameGuiObjects += (int)setRenderObjects.size();
