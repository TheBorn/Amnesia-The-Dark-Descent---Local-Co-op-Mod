...
namespace hpl {
	...
	void cEngine::CheckIfAppInFocusElseWait()
	{
		...
		////////////////////////////////////////////////////////////////////////
		// Wait on VISIBILITY, not on keyboard focus.
		//
		// This used to spin on GetWindowInputFocus(). Anything that took focus for a
		// moment -- an overlay, a notification, a click on another monitor, an input
		// grab toggling -- therefore stopped the render loop dead: no draw, no buffer
		// swap, for as long as it lasted. The window event log caught real gaps of 5.3
		// and 8.6 seconds with not a single frame presented in between, and the frame
		// heartbeat confirms it by having no entry at all across those spans.
		//
		// On a windowed GL context that is a black screen which heals itself the moment
		// focus returns -- flicker, black for a few seconds, flicker, normal. No probe
		// inside the renderer could ever have seen it, because the renderer was not
		// running.
		//
		// A window that is merely unfocused is still on screen and still has to be
		// drawn. Only a hidden or minimised one has nothing to show, and that is what
		// this waits for now. Alt-tabbing away still parks the game; being brushed past
		// by something that wants focus for a second no longer blanks it.
		while(	pllGfx->GetWindowIsVisible()==false)
		{
			cPlatform::Sleep(100);
