...
namespace hpl {
	...
	bool cLanguageFile::EntryExists(const tString& asCat, const tString& asName)
	{
		tLanguageCategoryMapIt CatIt = m_mapCategories.find(asCat);
		if(CatIt == m_mapCategories.end()) return false;

		cLanguageCategory *pCategory = CatIt->second;

		return pCategory->m_mapEntries.find(asName) != pCategory->m_mapEntries.end();
	}
