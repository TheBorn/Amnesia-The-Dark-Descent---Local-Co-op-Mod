...
namespace hpl {
	...
	static tWString gsLogFilePath;

	void SetLogFile(const tWString &asFile)
	{
		gsLogFilePath = asFile;
		gLogWriter.SetFileName(asFile);
	}

	const tWString& GetLogFilePath()
	{
	...
	static bool gbLogVerboseActive = false;

	void SetLogVerbose(bool abX)
	{
		gbLogVerboseActive = abX;
	}

	bool GetLogVerbose()
	{
		return gbLogVerboseActive;
	}

	void LogVerbose(const char* fmt, ...)
	{
		if(gbLogVerboseActive==false) return;
