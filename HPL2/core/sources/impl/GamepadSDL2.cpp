...
namespace hpl {
	...
	void cGamepadSDL2::Update()
	{
		...
		// mlstAxisChanges was the ONE queue not cleared here, and analog sticks
		// emit axis events continuously -- so it grew for the entire session.
		// Nothing drains it during gameplay (only the key-config binding screen
		// pops from it), and GetInputUpdate() calls mlstAxisChanges.remove(),
		// which is an O(n) scan of the whole list. Cost per frame therefore grew
		// without bound the longer a stick was being moved -- micro-stutter that
		// builds up, gated on a controller being connected and in use, and gone
		// the moment the pad is unplugged.
		//
		// Cleared per frame now, exactly like the other three queues: the binding
		// screen reads it in the same frame the input arrives, same as it already
		// does with mlstInputUpdates.
		mlstAxisChanges.clear();
	...
	void cGamepadSDL2::SetAxisDeadZoneRadiusValue(float afValue)
	{
		...
		//The paired-thumbstick path tests against the SQUARED radius, which was
		//never recomputed here -- so callers lowering the dead zone (coop sets
		//0.15) only affected single axes (triggers) while the sticks silently
		//kept the 0.333 default. Keep the two in lockstep.
		mfDeadZoneRadiusSqr = afValue * afValue;
	}
