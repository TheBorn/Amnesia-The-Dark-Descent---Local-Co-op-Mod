...
#   endif
		...
		mbInitHasBeenRun = true;
			ImGuiManager::Init(mpScreen, mGLContext);
    ...
    {
#if SDL_VERSION_ATLEAST(2, 0, 0)
        //SHOWN stays set on a minimised window, so it has to be excluded explicitly
        //or "visible" would be true for a window that is nothing but a taskbar entry.
        const Uint32 lFlags = SDL_GetWindowFlags(mpScreen);
        return (lFlags & SDL_WINDOW_SHOWN) != 0 && (lFlags & SDL_WINDOW_MINIMIZED) == 0;
#else
	...
	//////////////////////////////////////////////////////////////////////////
	// BLACK FRAME PROBE -- temporary, remove once the flicker is closed out
	//////////////////////////////////////////////////////////////////////////

	//Latched over in cScene::Render.
	extern int gLuxDbgVisibleViewports;
	extern int gLuxDbgWorldRenders;
	extern int gLuxDbgFramesDropped;

	//Writes the back buffer as an uncompressed 24-bit TGA next to hpl.log. TGA because
	//it needs no library and its default origin is bottom-left, which is exactly what
	//glReadPixels hands back -- no flip, nothing to get wrong.
	static void gLuxDumpBackBuffer(int alW, int alH, int alIndex)
	{
		glReadBuffer(GL_BACK);

		std::vector<unsigned char> vPixels((size_t)alW*(size_t)alH*3);
		glReadPixels(0,0,alW,alH, GL_BGR, GL_UNSIGNED_BYTE, &vPixels[0]);

		tWString sDir = GetLogFilePath();
		size_t lSlash = sDir.find_last_of(_W("/\\"));
		sDir = (lSlash==tWString::npos) ? tWString() : sDir.substr(0,lSlash+1);

		tWString sPath = sDir + _W("blackframe") + cString::ToStringW(alIndex) + _W(".tga");

		FILE *pFile = cPlatform::OpenFile(sPath, _W("wb"));
		if(pFile==NULL){ Error("Could not open black frame dump for writing\n"); return; }

		unsigned char header[18];
		for(int i=0; i<18; ++i) header[i] = 0;
		header[2]  = 2;							//uncompressed true colour
		header[12] = (unsigned char)(alW & 0xFF);
		header[13] = (unsigned char)((alW >> 8) & 0xFF);
		header[14] = (unsigned char)(alH & 0xFF);
		header[15] = (unsigned char)((alH >> 8) & 0xFF);
		header[16] = 24;						//bits per pixel

		fwrite(header, 1, 18, pFile);
		fwrite(&vPixels[0], 1, vPixels.size(), pFile);
		fclose(pFile);

		Log("  wrote blackframe%d.tga\n", alIndex);
	}

	//////////////////////////////////////////////////////////////////////////
	// FLIGHT RECORDER -- records EVERY presented frame, no trigger thresholds.
	//
	// The probes above and below never caught the live flicker because each had a
	// structural blind spot: the collapse test needs the previous frame at >=40
	// brightness (dark corridors never qualify, and in the menu the 3D background
	// holds the scanline max up forever, so a GUI-only dropout can never trip it),
	// and the DARK QUAD probe in cGuiSet::DrawGfx read the material ARGUMENT,
	// which every fade/death/insanity quad leaves at the default. This one records
	// unconditionally into a ring buffer; F9 dumps the last ~20 seconds as CSV
	// next to hpl.log, and a sharp RELATIVE collapse of any cell auto-dumps too.
	// One repro session = one attributable trace.
	struct cLuxFrameTraceEntry
	{
		unsigned long mlTimeMs;
		int mvCells[6];			//scanline maxes: rows 30/50/70%, left|right halves
		int mlLeakedFbo;		//FBO left bound at end of frame (0 = back buffer)
		int mlImGuiLists;
		int mlImGuiVerts;
		int mlVisibleViewports;
		int mlWorldRenders;
		int mlGuiObjects;		//gui render objects rendered this frame (all sets)
		float mfMinQuadMul;		//darkest full-screen gui quad this frame (1 = none)
		char msQuadSet[20];		//set that drew it
	};

	static const int kLuxFrameTraceSize = 1200;
	static cLuxFrameTraceEntry gvLuxFrameTrace[kLuxFrameTraceSize];
	static int glLuxFrameTraceIdx = 0;
	static int glLuxFrameTraceCount = 0;

	//Set by ImGuiManager::ProcessEvent on F9. Defined here, extern'd there.
	bool gLuxFrameTraceDumpRequested = false;

	//Per-frame gui stats, written by cGuiSet during draws, reset after recording.
	extern int gLuxDbgFrameGuiObjects;
	extern float gLuxDbgFrameMinQuadMul;
	extern char gLuxDbgFrameQuadSet[20];

	static void gLuxDumpFrameTrace(const char* asReason)
	{
		tWString sDir = GetLogFilePath();
		size_t lSlash = sDir.find_last_of(_W("/\\"));
		sDir = (lSlash==tWString::npos) ? tWString() : sDir.substr(0,lSlash+1);

		tWString sPath = sDir + _W("frametrace_") +
						cString::ToStringW((int)cPlatform::GetApplicationTime()) + _W(".csv");

		FILE *pFile = cPlatform::OpenFile(sPath, _W("w"));
		if(pFile==NULL){ Error("Could not open frame trace for writing\n"); return; }

		fprintf(pFile, "# reason: %s\n", asReason);
		fprintf(pFile, "ms,c0L,c0R,c1L,c1R,c2L,c2R,leakedFbo,imguiLists,imguiVerts,"
						"viewports,worldRenders,guiObjects,minQuadMul,quadSet\n");

		int lCount = glLuxFrameTraceCount < kLuxFrameTraceSize ? glLuxFrameTraceCount : kLuxFrameTraceSize;
		int lStart = glLuxFrameTraceCount < kLuxFrameTraceSize ? 0 : glLuxFrameTraceIdx;
		for(int i=0; i<lCount; ++i)
		{
			const cLuxFrameTraceEntry &e = gvLuxFrameTrace[(lStart + i) % kLuxFrameTraceSize];
			fprintf(pFile, "%lu,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.3f,%s\n",
				e.mlTimeMs,
				e.mvCells[0],e.mvCells[1],e.mvCells[2],e.mvCells[3],e.mvCells[4],e.mvCells[5],
				e.mlLeakedFbo, e.mlImGuiLists, e.mlImGuiVerts,
				e.mlVisibleViewports, e.mlWorldRenders, e.mlGuiObjects,
				e.mfMinQuadMul, e.msQuadSet);
		}
		fclose(pFile);
		Log("[frame trace] wrote %d frames (%s)\n", lCount, asReason);
	}

	//Runs immediately before the flip -- after cScene::Render AND after ImGui -- which
	//is the only point in the frame that measures what the player actually sees. The
	//previous probe sat at the end of cScene::Render and so was blind to everything
	//after it; it reported a clean frame every time while the screen was flickering.
	//
	//Three scanlines, whole-frame maximum, and it fires only on a real collapse: the
	//previous frame clearly lit, this one nearly black. The old per-row "dropped to a
	//third" test is gone -- that is what produced entries with rows still at 255.
	static void gLuxBlackFrameProbe(int alW, int alH, int alPrevDrawFbo)
	{
		static int glPrevMax = 255;
		static int glDumpsWritten = 0;
		static int glDumpNextFrame = 0;
		static unsigned long glLastLogMs = 0;
		static std::vector<unsigned char> gvRow;

		if(alW < 16 || alH < 16) return;
		if((int)gvRow.size() < alW*4) gvRow.resize(alW*4);

		//Queued by the previous frame's hit -- see the note at the bottom.
		if(glDumpNextFrame > 0)
		{
			gLuxDumpBackBuffer(alW, alH, 100 + glDumpNextFrame);
			glDumpNextFrame = 0;
		}

		glReadBuffer(GL_BACK);

		//SIX CELLS, not one number.
		//
		// Taking the max of a whole scanline cannot see one viewport go black while
		// the other stays lit -- the lit half holds the maximum up and the collapse is
		// invisible. That is precisely the shape split-screen produces, and it is why
		// this reported "no black frames" through a session full of them.
		//
		// Three rows x two halves covers both layouts: left/right splits differ across
		// the halves, top/bottom splits differ across the rows.
		static int slPrevCell[6] = {255,255,255,255,255,255};

		int lCell[6] = {0,0,0,0,0,0};
		int lMax = 0;
		const int lHalf = alW/2;

		for(int r=0; r<3; ++r)
		{
			//30% / 50% / 70% of the height, not 25/50/75.
			//
			// Split views are letterboxed 16:9 inside their half, so the top and bottom
			// of each half are permanently black bars. Rows at a quarter and three
			// quarters land right on those edges and read 0 whenever the content shifts
			// a few pixels -- which fired constantly and masked the real event.
			const int lY = (alH * (30 + r*20)) / 100;
			glReadPixels(0, lY, alW, 1, GL_RGBA, GL_UNSIGNED_BYTE, &gvRow[0]);

			for(int x=0; x<alW; ++x)
			{
				const int i = x*4;
				int v = gvRow[i+0];
				if(gvRow[i+1] > v) v = gvRow[i+1];
				if(gvRow[i+2] > v) v = gvRow[i+2];

				const int lIdx = r*2 + (x < lHalf ? 0 : 1);
				if(v > lCell[lIdx]) lCell[lIdx] = v;
				if(v > lMax) lMax = v;
			}
		}

		//////////////////////////////////////////////////////////////////
		// FLIGHT RECORDER: record this frame unconditionally, then check the
		// dump triggers. Runs before the old collapse analysis so the trace
		// exists whether or not any of the legacy thresholds fire.
		{
			int lTraceImGuiLists = 0, lTraceImGuiVerts = 0;
			ImGuiManager::GetLastDrawStats(lTraceImGuiLists, lTraceImGuiVerts);

			//Relative collapse against the PREVIOUS recorded frame -- no absolute
			//floor, so it works in pitch-dark corridors and menu overlays alike.
			bool bAutoDump = false;
			if(glLuxFrameTraceCount > 0)
			{
				const cLuxFrameTraceEntry &prev =
					gvLuxFrameTrace[(glLuxFrameTraceIdx + kLuxFrameTraceSize - 1) % kLuxFrameTraceSize];
				for(int c=0; c<6; ++c)
				{
					if(prev.mvCells[c] >= 25 && lCell[c] <= prev.mvCells[c]/8)
					{
						bAutoDump = true;
						break;
					}
			...
			cLuxFrameTraceEntry &e = gvLuxFrameTrace[glLuxFrameTraceIdx];
			e.mlTimeMs = cPlatform::GetApplicationTime();
			for(int c=0; c<6; ++c) e.mvCells[c] = lCell[c];
			e.mlLeakedFbo = alPrevDrawFbo;
			e.mlImGuiLists = lTraceImGuiLists;
			e.mlImGuiVerts = lTraceImGuiVerts;
			e.mlVisibleViewports = gLuxDbgVisibleViewports;
			e.mlWorldRenders = gLuxDbgWorldRenders;
			e.mlGuiObjects = gLuxDbgFrameGuiObjects;
			e.mfMinQuadMul = gLuxDbgFrameMinQuadMul;
			for(int c=0; c<20; ++c) e.msQuadSet[c] = gLuxDbgFrameQuadSet[c];
			e.msQuadSet[19] = 0;

			glLuxFrameTraceIdx = (glLuxFrameTraceIdx+1) % kLuxFrameTraceSize;
			++glLuxFrameTraceCount;

			//Reset the per-frame gui stats for the next frame's draws.
			gLuxDbgFrameGuiObjects = 0;
			gLuxDbgFrameMinQuadMul = 1.0f;
			gLuxDbgFrameQuadSet[0] = 0;

			static unsigned long slLastAutoDumpMs = 0;
			const unsigned long lDumpNow = e.mlTimeMs;

			if(gLuxFrameTraceDumpRequested)
			{
				gLuxFrameTraceDumpRequested = false;
				gLuxDumpFrameTrace("F9");
			}
			else if(bAutoDump && (slLastAutoDumpMs==0 || lDumpNow - slLastAutoDumpMs >= 15000))
			{
				slLastAutoDumpMs = lDumpNow;
				gLuxDumpFrameTrace("auto cell collapse");
			}
		}

		//ONE HALF AT ZERO WHILE THE OTHER IS LIT. Nothing else counts.
		//
		// A ratio test cannot work here. This game is played by lantern light and the
		// brightness of a half legitimately collapses five-fold when a player turns to
		// face a wall -- every hit so far has been exactly that, and the dumps show
		// ordinary dark rooms.
		//
		// But both halves look at the SAME WORLD. One of them being at pure zero while
		// the other is clearly lit is not a lighting condition -- it is a viewport that
		// did not put anything on screen, which is the reported symptom exactly and
		// nothing else produces it.
		int lLeftMax = lCell[0];  if(lCell[2] > lLeftMax) lLeftMax = lCell[2];
		if(lCell[4] > lLeftMax) lLeftMax = lCell[4];

		int lRightMax = lCell[1]; if(lCell[3] > lRightMax) lRightMax = lCell[3];
		if(lCell[5] > lRightMax) lRightMax = lCell[5];

		int lWorstCell = -1;
		if(lLeftMax  <= 6 && lRightMax >= 40) lWorstCell = 0;	//left half dead
		else if(lRightMax <= 6 && lLeftMax >= 40) lWorstCell = 1;	//right half dead

		//////////////////////////////////////////////////////////////////
		// Is the game still PRESENTING?
		//
		// This function runs once per SwapBuffers, so the gap between two calls is
		// the gap between two presented frames. At 60Hz that is ~16ms.
		//
		// The ImGui overlay draws immediately before the flip, after everything the
		// renderer does, so nothing inside the renderer can hide it. It disappearing
		// during a blackout means frames are not reaching the display -- and this
		// number says which of the only two reasons it is:
		//
		//   gaps of hundreds of ms  -> the loop stalled, we stopped presenting, and
		//                              the cause is in this process
		//   steady ~16ms throughout -> we presented every frame and the display did
		//                              not show them; nothing in this codebase can
		//                              fix that and I stop changing render code
		{
			static unsigned long slLastSwapMs = 0;
			const unsigned long lSwapNow = cPlatform::GetApplicationTime();

			if(slLastSwapMs != 0)
			{
				const unsigned long lGap = lSwapNow - slLastSwapMs;
				if(lGap >= 100)
					Log("[present gap] %lu ms with no frame presented, ending at %lu ms\n",
						lGap, lSwapNow);

				//////////////////////////////////////////////////////////////
				// Frame PACING, not just stalls.
				//
				// Everything measured so far says the game hands over a correct, lit
				// frame on schedule while the screen is black -- including the ImGui
				// overlay, which is drawn microseconds before the flip and cannot be
				// hidden by anything in the renderer.
				//
				// If that is true then what is left is presentation, and the way co-op
				// causes it WITHOUT any co-op code being wrong is frame pacing: two full
				// deferred passes per frame instead of one, so frame times roughly double
				// and swing far harder against a 60Hz vsync boundary. Sustained swinging
				// between one and two refresh intervals is the classic VRR/flip-mode
				// renegotiation condition, and it looks exactly like rapid flicker, a
				// black period, more flicker, then recovery.
				//
				// This measures it. Steady ~16ms means pacing is innocent and the cause is
				// still inside this process. Alternating 16/33 with a high swing count
				// means it is not, and no further renderer change can help.
				static unsigned long slPaceStartMs = 0;
				static unsigned long slPaceMin = 100000, slPaceMax = 0, slPaceSum = 0;
				static int slPaceCount = 0, slPaceSwings = 0;
				static unsigned long slPacePrevGap = 0;

				if(lGap < 100)
				{
					if(lGap < slPaceMin) slPaceMin = lGap;
					if(lGap > slPaceMax) slPaceMax = lGap;
					slPaceSum += lGap; ++slPaceCount;

					//A swing is one frame differing from the last by more than half a
					//refresh interval -- the thing a variable-refresh panel has to chase.
					const long lDelta = (long)lGap - (long)slPacePrevGap;
					if(lDelta > 8 || lDelta < -8) ++slPaceSwings;
					slPacePrevGap = lGap;
				}

				if(slPaceStartMs == 0) slPaceStartMs = lSwapNow;
				else if(lSwapNow - slPaceStartMs >= 10000 && slPaceCount > 0)
				{
					Log("[pacing] last 10s: %d frames, %lu..%lu ms (avg %lu), %d swings >8ms (%d%%)\n",
						slPaceCount, slPaceMin, slPaceMax, slPaceSum/(unsigned long)slPaceCount,
						slPaceSwings, (slPaceSwings*100)/slPaceCount);

					slPaceStartMs = lSwapNow; slPaceMin = 100000; slPaceMax = 0;
					slPaceSum = 0; slPaceCount = 0; slPaceSwings = 0;
				}
			}
			slLastSwapMs = lSwapNow;
		}

		const int lPrev = glPrevMax;
		glPrevMax = lMax;

		//A heartbeat, so the threshold above can be checked against reality instead of
		//assumed. If these lines sit in the 40s all through the dark parts then the old
		//floor of 96 was throwing away every hit, and that is worth knowing from the log
		//rather than by inference. One line per ten seconds.
		{
			static unsigned long slLastBeatMs = 0;
			static int slBeatMin = 255, slBeatMax = 0;

			if(lMax < slBeatMin) slBeatMin = lMax;
			if(lMax > slBeatMax) slBeatMax = lMax;

			const unsigned long lBeatNow = cPlatform::GetApplicationTime();
			if(slLastBeatMs == 0) slLastBeatMs = lBeatNow;
			else if(lBeatNow - slLastBeatMs >= 10000)
			{
				Log("[frame brightness] last 10s: min %d, max %d\n", slBeatMin, slBeatMax);
				slLastBeatMs = lBeatNow; slBeatMin = 255; slBeatMax = 0;
			}
		}

		//RATIO, not absolute levels.
		//
		//The first version demanded the previous frame peak above 96 and this one fall
		//below 24. In a game played by lantern light that is nonsense: three scanlines
		//through a dim corridor peak in the 40s, so the test could never fire anywhere
		//dark -- which is most of the game. It caught the lit main menu and nothing else,
		//and two whole sessions of "it went black" produced no entries.
		//
		//A collapse to a fifth of the previous frame, from anything that was visible at
		//all, is the actual event. Three widely separated scanlines all losing 80% of
		//their brightness in one frame is not a light going out or a corner being turned;
		//nothing in a scene moves that fast across the whole picture at once.
		//Whole frame: everything at zero when the frame before was clearly lit.
		const bool bWholeFrameCollapse = (lPrev >= 40 && lMax <= 6);

		for(int c=0; c<6; ++c) slPrevCell[c] = lCell[c];

		if(bWholeFrameCollapse==false && lWorstCell < 0) return;

		const unsigned long lNow = cPlatform::GetApplicationTime();
		//150ms, not 400. A flicker is a burst; at 400ms only the first frame of one
		//was ever recorded and the shape of it was invisible.
		if(glLastLogMs != 0 && lNow - glLastLogMs < 150) return;
		glLastLogMs = lNow;

		GLboolean bScissor = glIsEnabled(GL_SCISSOR_TEST);
		GLboolean bBlend   = glIsEnabled(GL_BLEND);
		GLint lScissorBox[4] = {0,0,0,0};	glGetIntegerv(GL_SCISSOR_BOX, lScissorBox);
		GLint lViewport[4]   = {0,0,0,0};	glGetIntegerv(GL_VIEWPORT, lViewport);
		GLboolean bMask[4]   = {0,0,0,0};	glGetBooleanv(GL_COLOR_WRITEMASK, bMask);

		int lImGuiLists = 0, lImGuiVerts = 0;
		ImGuiManager::GetLastDrawStats(lImGuiLists, lImGuiVerts);

		Log("=== %s at %lu ms (frame max %d -> %d) ===\n",
			lWorstCell==0 ? "LEFT HALF DEAD" : (lWorstCell==1 ? "RIGHT HALF DEAD" : "WHOLE FRAME BLACK"),
			lNow, lPrev, lMax);
		Log("  cells now (row0 L/R, row1 L/R, row2 L/R): %d %d | %d %d | %d %d\n",
			lCell[0], lCell[1], lCell[2], lCell[3], lCell[4], lCell[5]);
		Log("  scene: %d visible viewports, %d world renders, %d frames dropped so far\n",
			gLuxDbgVisibleViewports, gLuxDbgWorldRenders, gLuxDbgFramesDropped);
		Log("  imgui: %d cmd lists, %d verts   (0 verts = ImGui drew nothing this frame)\n",
			lImGuiLists, lImGuiVerts);
		Log("  gl: fboBeforeProbe=%d scissor=%d box=(%d,%d %dx%d) viewport=(%d,%d %dx%d)\n",
			alPrevDrawFbo, (int)bScissor,
			lScissorBox[0], lScissorBox[1], lScissorBox[2], lScissorBox[3],
			lViewport[0], lViewport[1], lViewport[2], lViewport[3]);
		Log("  gl: blend=%d colormask=%d%d%d%d screen=%dx%d\n",
			(int)bBlend, (int)bMask[0], (int)bMask[1], (int)bMask[2], (int)bMask[3], alW, alH);

		//Four pictures is plenty and keeps this from filling the disk. A picture settles
		//in one glance what the numbers argue about: black with a HUD on it, black with
		//nothing at all, and a dark full-screen quad over a live frame are three
		//different bugs.
		if(glDumpsWritten >= 6) return;
		++glDumpsWritten;
		gLuxDumpBackBuffer(alW, alH, glDumpsWritten);

		//And the one straight after it. Two consecutive frames settle what the single
		//dump could not: if the black frame is the next one multiplied by a constant,
		//something scaled a finished image -- a quad, a blend, a post effect. If the
		//ratio wanders across the picture instead, the scene itself rendered dark and
		//the lighting is what failed. Those are opposite bugs and nothing short of the
		//pair distinguishes them.
		glDumpNextFrame = glDumpsWritten;
	}
	...
	{
		//////////////////////////////////////////////////////////////////////
		// Capture what the frame LEFT bound, then bind the real back buffer
		// BEFORE ImGui draws.
		//
		// ImGui used to render into whatever FBO the last pass happened to leave
		// active. Several paths exit the frame with an internal FBO still bound
		// (a reflection sub-pass, a post effect deactivating mid-frame, a
		// no-renderer viewport), and on those frames the whole debug overlay
		// silently landed offscreen -- "even the imgui debug menu is affected".
		// The leaked binding is still captured first and recorded per frame in
		// the flight recorder, so the evidence is preserved; the overlay just no
		// longer pays for it.
		GLint lDrawFbo = 0; glGetIntegerv(GL_FRAMEBUFFER_BINDING_EXT, &lDrawFbo);
...

		{
			...
			//The REAL drawable, not mvScreenSize. In dual-monitor coop the window is
			//5120 wide while mvScreenSize still reads 2560, so the probe was scanning
			//the left monitor only and would have missed anything that went black on
			//Player 2's half entirely. A measurement I am drawing conclusions from must
			//not be blind to half the screen.
			int lProbeW = mvScreenSize.x, lProbeH = mvScreenSize.y;
			if(mpScreen)
			{
				int lRealW = 0, lRealH = 0;
				SDL_GetWindowSize(mpScreen, &lRealW, &lRealH);
				if(lRealW > 0 && lRealH > 0){ lProbeW = lRealW; lProbeH = lRealH; }
			}

			//Verbose only. The probe costs three synchronous glReadPixels every
			//frame -- it stalls the pipeline by design, which is the whole reason
			//Options -> Debug warns that verbose logging is expensive.
			if(GetLogVerbose())
			{
				gLuxBlackFrameProbe(lProbeW, lProbeH, (int)lDrawFbo);
			}
			else
			{
				//The probe owns the per-frame reset of these, but the code that
				//WRITES them runs every frame regardless -- left unreset the
				//object count would climb until it overflowed and the minimum
				//quad multiplier would stick at the session low.
				gLuxDbgFrameGuiObjects = 0;
				gLuxDbgFrameMinQuadMul = 1.0f;
				gLuxDbgFrameQuadSet[0] = 0;

				//And an F9 pressed with verbose off must not sit latched, ready
				//to fire a trace dump the moment verbose is switched on.
				gLuxFrameTraceDumpRequested = false;
			}
		}
#if SDL_VERSION_ATLEAST(2, 0, 0)
