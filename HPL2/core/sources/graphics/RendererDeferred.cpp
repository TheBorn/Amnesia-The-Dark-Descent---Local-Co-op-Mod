...
namespace hpl {
	...
	void cRendererDeferred::CopyToFrameBuffer()
	{
		...
		//Use a flat projection sized to the viewport, draw a quad filling
		//the viewport, and sample the entire accumulation buffer.
		cVector2f vViewportSize((float)mvRenderTargetSize.x, (float)mvRenderTargetSize.y);
		SetFlatProjection(vViewportSize);

		//Sample only the part of the accumulation buffer that was actually drawn to.
		//At scale 1 this is (0,0)..(screen size), i.e. the whole buffer as before.
		cVector2f vSrcMinUV, vSrcMaxUV;
		GetScaledUVRange(mvScreenSizeFloat, vSrcMinUV, vSrcMaxUV);

		DrawQuad(	cVector2f(0,0), vViewportSize,
					vSrcMinUV, 
					vSrcMaxUV,
					true);
	...
	{
		// This renderer's internal buffers are ALWAYS the full screen, and every
		// internal SetFrameBuffer() below uses the default abUsePosAndSize=false,
		// so the GL viewport is the whole buffer for every pass. A split viewport
		// is rasterised at full screen resolution and only shrunk into its
		// on-screen rect afterwards, by CopyToFrameBuffer from EndRendering().
		//
		// So for the duration of this function the render target IS the full
		// screen, and mvPos/mvRenderTargetSize have to say so. Everything derived
		// from them describes pixels in those full-screen buffers:
		//
		//   - light scissor rects
		//   - spot light and reflection clip rects
		//   - the refraction clip rect, and the destination offset of the
		//     refraction copy in CopyFrameBufferToTexure
		//
		// Leaving them as the viewport's on-screen rect is the identity for a
		// viewport at 0,0 covering the window -- single player, and P1 in
		// dual-monitor layout -- so it looks harmless. For P2 it is off by a whole
		// screen width: light scissors cut away real lighting, and the refraction
		// copy is written a screen to the right of where the water shader samples
		// it, so P2's water reads back the region still holding P1's copy.
		//
		// NOT a toggle. There is no correct alternative setting.
		cVector2l vSavedRenderTargetPos = mpCurrentRenderTarget->mvPos;
		cVector2l vSavedRenderTargetSize = mvRenderTargetSize;

		// Render scale shrinks the rasterised area of every buffer by one factor,
		// anchored top-left. The render target then IS that sub-rect: it is what
		// scissor rects, clip rects and the refraction copy all have to describe.
		ApplyRenderScale();

		// A REFLECTION pass writes the quarter-size reflection buffers, not the
		// screen-size ones. Claiming the full screen here made every light scissor
		// rect inside a reflection four times too big for the buffer it was applied
		// to, so the reflection came out as whatever survived the clamp. Water is
		// the only thing in the game that reads that buffer.
		SetupInternalBufferRenderTarget();
		...
		// Restore, because CopyToFrameBuffer genuinely does need the on-screen rect:
		// that is the one pass that maps the full-screen result into the viewport.
		mpCurrentRenderTarget->mvPos = vSavedRenderTargetPos;
		mvRenderTargetSize = vSavedRenderTargetSize;
	}
	...
	void cRendererDeferred::RenderSSAO()
	{
		...
		//Under a scaled render only the top-left of each of these holds this frame.
		//Identity at scale 1. vSSAOSize itself is left alone where it is fed to the
		//shader as avScreenSize -- that describes the BUFFER, which has not changed.
		cVector2f vGBufferDepthMinUV, vGBufferDepthMaxUV;
		GetScaledUVRange(vGBufferDepthSize, vGBufferDepthMinUV, vGBufferDepthMaxUV);
		cVector2f vSSAOMinUV, vSSAOMaxUV;
		GetScaledUVRange(vSSAOSize, vSSAOMinUV, vSSAOMaxUV);
		...
		DrawQuad(vQuadPos, vQuadSize, vGBufferDepthMinUV, vGBufferDepthMaxUV,true);
		...
		DrawQuad(vQuadPos, vQuadSize, vSSAOMinUV, vSSAOMaxUV,true);
		...
		if(false)
		{
			...
			SetTexture(0,mpSSAOTexture);
			DrawQuad(0,1, vSSAOMinUV, vSSAOMaxUV, true);
	...
	void cRendererDeferred::RenderEdgeSmooth()
	{
		...
		//Scaled render: only the top-left of the screen-sized buffers is this frame.
		cVector2f vScreenMinUV, vScreenMaxUV;
		GetScaledUVRange(mvScreenSizeFloat, vScreenMinUV, vScreenMaxUV);
		...
		DrawQuad(vQuadPos, vQuadSize, vScreenMinUV, vScreenMaxUV,true);

		////////////////////////////////////////////
		// Copy the screen to temp texture
		SetAccumulationBuffer();
		CopyFrameBufferToTexure(mpEdgeSmooth_TempAccum,0,GetScaledSize(mvScreenSize),0, true);
	...
	void cRendererDeferred::InitLightRendering()
	{
		...
		//This quad's UVs are baked once in LoadData to 0..screen size. Outside the
		//FOV guard because the render scale can change without the frustum moving,
		//and at scale 1 this writes back the same numbers CreateQuadVertexBuffer did.
		{
			cVector2f vLightMinUV, vLightMaxUV;
			GetScaledUVRange(mvScreenSizeFloat, vLightMinUV, vLightMaxUV);
			UpdateQuadVertexUV(mpFullscreenLightQuad, vLightMinUV, vLightMaxUV, true, true);
		}
	...
	void cRendererDeferred::RenderLights_StencilBack_ScreenQuad()
	{
		...
		{
			ClearFrameBuffer(eClearFrameBufferFlag_Stencil,false);
			mbStencilNeedClearing = false;
		}
	...
	void cRendererDeferred::RenderLights()
	{
		...
		//Set up frame buffer
		SetAccumulationBuffer();
		ClearFrameBuffer(eClearFrameBufferFlag_Color |eClearFrameBufferFlag_Stencil,false); //Perhaps there is some ambient light that can be added instead?
	...
	void cRendererDeferred::RenderSubMeshEntityReflection(cSubMeshEntity *pReflectionObject)
	{
		...
		///////////////////////////////////
		//Make render target
		// THE SPLIT-SCREEN WATER BUG.
		//
		// This used to be the viewport's ON-SCREEN rect divided by the reflection
		// size divisor. That reads as sensible and is wrong the moment a viewport
		// does not start at 0,0: the reflection is rendered into a buffer that is
		// mvScreenSize/div in its entirety, so its render target is that WHOLE
		// buffer at the origin -- it has nothing to do with where the viewport
		// happens to sit on screen.
		//
		// P2 in dual-monitor sits at x = one monitor's width, so its reflection
		// target landed at x = that width / 4, which is past the right edge of a
		// buffer only a quarter as wide. P2's reflection was rendered off the end
		// of the buffer and the water sampled whatever was left in it -- which is
		// P1's reflection, rendered moments earlier into the same shared texture.
		// P1 at x=0 divided to 0 and looked perfect, so it read as a P2-only bug.
		cRenderTarget renderTarget;
		renderTarget.mpFrameBuffer = mpReflectionBuffer;
		renderTarget.mvPos = cVector2l(0, 0);
		renderTarget.mvSize = mvScreenSize / mlReflectionSizeDiv;
		...
		if(mpCurrentSettings->mbClipReflectionScreenRect)
		{
			...
			if(bVisible)
			{
				...
				if(bNeedsClipRect)
				{
					// THE RENDER SCALE REFLECTION BUG.
					//
					// This measured the surface against the WHOLE reflection buffer, but
					// the pass about to run rasterises into a render-scaled sub-rect of
					// it. Both the rect's position and its size are in the unscaled space,
					// so at scale 0.5 a surface whose fragments land on rows 100..150 got
					// a scissor covering rows 200..300 -- no overlap at all, and the
					// reflection was clipped away entirely for anything not hard against
					// the top-left corner. Identity at scale 1, which is why it never
					// showed up before the slider was touched.
					//
					// Same source as the render target the nested pass will set, so the
					// rect and the thing it clips cannot disagree.
					cVector2l vRenderTargetSize = GetInternalBufferRenderSize(true);
		...
		// THE SPLIT-SCREEN WATER BUG.
		//
		// That BeginRendering re-runs InitAndResetRenderFunctions, which recomputes
		// mvRenderTargetSize from the viewport's ON-SCREEN rect -- undoing the setup
		// RenderObjects did at the top of the pass. Everything drawn after the first
		// reflective surface in the frame then measures the full-screen internal
		// buffers with the viewport's dimensions, including the refraction clip rect
		// and the copy that fills the refraction texture from it.
		//
		// P2's viewport is SMALLER than the buffers, so its refraction copy grabbed
		// the wrong region and the water sampled texels nothing had refreshed this
		// frame -- which still held P1's copy from moments earlier. P1's viewport is
		// the whole window, so its rect came out too BIG, got clamped, over-copied,
		// and covered the water anyway. Hence: correct for P1, P1's world showing
		// through P2's water, and only ever on surfaces that reflect.
		//
		// THE RENDER SCALE CROP.
		//
		// That same BeginRendering also resets mvRenderScale to 1.0, and restoring
		// the render target SIZE without the scale it is computed from just makes
		// SetupInternalBufferRenderTarget below recompute it at full size. From the
		// first reflective surface in a frame onward the rest of the pass then
		// rasterised full-buffer, and CopyToFrameBuffer sampled the WHOLE
		// accumulation buffer -- while the opaque world was really sitting in the
		// top-left sub-rect. The view shrank into a corner of the viewport, and only
		// at angles where something reflective was visible.
		//
		// Order matters: scale first, then the render target derived from it.
		ApplyRenderScale();
		SetupInternalBufferRenderTarget();
	...
	void cRendererDeferred::ApplyRenderScale()
	{
		// One place, so the top of the pass and the restore after a reflection can
	...
	cVector2l cRendererDeferred::GetInternalBufferRenderSize(bool abIsReflection)
	{
		cVector2l vSize = GetScaledSize(mvScreenSize);

		// A REFLECTION pass writes the smaller reflection buffers, not the
		// screen-size ones.
		//
		// Divide the ALREADY-SCALED main size rather than scaling the divided one.
		// The water shader turns a main-pass pixel into a reflection texel by
		// dividing by this same divisor, so the reflection's rasterised area has to
		// be exactly that -- scaling and dividing in the other order rounds twice
		// and puts the last row and column a pixel out.
		if(abIsReflection)
		{
			vSize = vSize / mlReflectionSizeDiv;
	...
	void cRendererDeferred::SetupInternalBufferRenderTarget()
	{
		mpCurrentRenderTarget->mvPos = cVector2l(0, 0);
		mvRenderTargetSize = GetInternalBufferRenderSize(mpCurrentSettings->mbIsReflection);
	}
	...
	{
		if(mpCurrentSettings->mbIsReflection)
			SetFrameBuffer(mpReflectionBuffer);
		else
			SetFrameBuffer(mpAccumBuffer);
	}
	...
	{
		int lType = mpCurrentSettings->mbIsReflection ? 1 : 0;
		SetFrameBuffer(mpGBuffer[lType][aComponents]);
	}
