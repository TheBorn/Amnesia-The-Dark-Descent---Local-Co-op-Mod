...
namespace hpl {
	...
	void iRenderFunctions::SetupRenderFunctions(iLowLevelGraphics *apLowLevelGraphics)
	{
		...
		mvRenderScale = cVector2f(1.0f, 1.0f);
	}

	//-----------------------------------------------------------------------

	void iRenderFunctions::SetRenderScale(const cVector2f& avScale)
	{
		mvRenderScale = avScale;

		//////////////////////////////////////////////////////////////////
		// Snapped to sixteenths.
		//
		// The renderer works in several buffer sizes at once -- full, the
		// half-res SSAO pair, the quarter-res reflection -- and each rounds the
		// scaled size against its OWN dimensions. An arbitrary scale like 0.37
		// lets those roundings disagree by a texel, so the depth sub-rect and
		// the SSAO sub-rect stop describing the same part of the screen and the
		// effect samples just outside what was drawn. Sixteenths divide cleanly
		// through /2 and /4 at every resolution the game is likely to run at.
		mvRenderScale.x = (float)((int)(mvRenderScale.x * 16.0f + 0.5f)) / 16.0f;
		mvRenderScale.y = (float)((int)(mvRenderScale.y * 16.0f + 0.5f)) / 16.0f;

		//Below about a quarter the intermediate buffers get small enough that the
		//half-res SSAO and quarter-res reflection targets start rounding to nothing.
		if(mvRenderScale.x < 0.25f) mvRenderScale.x = 0.25f;
		if(mvRenderScale.y < 0.25f) mvRenderScale.y = 0.25f;
		if(mvRenderScale.x > 1.0f) mvRenderScale.x = 1.0f;
		if(mvRenderScale.y > 1.0f) mvRenderScale.y = 1.0f;
	}

	//-----------------------------------------------------------------------

	cVector2l iRenderFunctions::GetScaledSize(const cVector2l& avSize) const
	{
		if(mvRenderScale.x >= 1.0f && mvRenderScale.y >= 1.0f) return avSize;

		cVector2l vOut;
		vOut.x = (int)((float)avSize.x * mvRenderScale.x + 0.5f);
		vOut.y = (int)((float)avSize.y * mvRenderScale.y + 0.5f);

		if(vOut.x < 1) vOut.x = 1;
		if(vOut.y < 1) vOut.y = 1;
		if(vOut.x > avSize.x) vOut.x = avSize.x;
		if(vOut.y > avSize.y) vOut.y = avSize.y;

		return vOut;
	}

	cVector2f iRenderFunctions::GetScaledSizeF(const cVector2f& avSize) const
	{
		cVector2l vScaled = GetScaledSize(cVector2l((int)avSize.x, (int)avSize.y));
		return cVector2f((float)vScaled.x, (float)vScaled.y);
	}

	void iRenderFunctions::GetScaledUVRange(const cVector2f& avTexSize, cVector2f& avMinUV, cVector2f& avMaxUV) const
	{
		cVector2l vScaled = GetScaledSize(cVector2l((int)avTexSize.x, (int)avTexSize.y));

		//Sub-rect sits at the TOP of the buffer; these UVs are BOTTOM-origin.
		avMinUV = cVector2f(0.0f, avTexSize.y - (float)vScaled.y);
		avMaxUV = cVector2f((float)vScaled.x, avTexSize.y);

		//////////////////////////////////////////////////////////////////
		// Half a texel in, on the two edges that border buffer nobody wrote.
		//
		// Only part of the buffer is rasterised when scaled; the rest still holds
		// whatever was there before, which is nothing -- black. A linearly
		// filtered tap sitting exactly ON the boundary averages the last real
		// texel with that black, and every full-screen pass that samples the
		// edge (bloom's blur taps above all, which walk outwards on purpose)
		// smears it inwards. That is the dark banding: not a pass computing the
		// wrong thing, a pass reading one row too far.
		//
		// Left and top edges are the buffer's own edges and are already clamped
		// by the sampler, so they are left alone.
		if(IsRenderScaled())
		{
			if(vScaled.x > 1) avMaxUV.x -= 0.5f;
			if(vScaled.y > 1) avMinUV.y += 0.5f;
		}
	}
														...
														const cVector2l& avGlobalScissorRectPos, const cVector2l& avGlobalScissorRectSize)
	{
		...
		//Off unless whoever is about to render explicitly asks for it. iRenderer is
		//shared by the simple and wireframe renderers too, and neither of them has
		//any of the sub-rect UV handling that makes a scaled render come out right.
		mvRenderScale = cVector2f(1.0f, 1.0f);
	}
	...
	void iRenderFunctions::ExitAndCleanUpRenderFunctions()
	{
		...
		if(mbUseGlobalScissorRect)
		{
			...
			//The low level is driven directly here, so the renderer-level cache has to
			//be told -- SetScissorActive below early-outs on it, and a cache that has
			//drifted from GL stays drifted because the guard that should fix it is the
			//thing being fooled.
			mbCurrentScissorActive = false;
		}
	}
	...
	void iRenderFunctions::SetFrameBuffer(iFrameBuffer *apFrameBuffer, bool abUsePosAndSize, bool abUseGlobalScissor)
	{
		...
		else if(IsRenderScaled())
		{
			//Scaled render: the pass still owns the buffer, but only its top-left
			//sub-rect is rasterised. Pos (0,0) with a smaller size puts the GL
			//viewport at the TOP of the buffer, which is where every sampler in the
			//renderer is told to look for it.
			cVector2l vFullSize = apFrameBuffer ? apFrameBuffer->GetSize() : mvScreenSize;
			cVector2l vScaledSize = GetScaledSize(vFullSize);

			if(mbLog){
				Log(" Setting FrameBuffer %d '%s', scaled size: %dx%d of %dx%d\n", apFrameBuffer,
					apFrameBuffer ? apFrameBuffer->GetName().c_str() : "",
					vScaledSize.x, vScaledSize.y, vFullSize.x, vFullSize.y);
			}
			mpLowLevelGraphics->SetCurrentFrameBuffer(apFrameBuffer, cVector2l(0,0), vScaledSize);
		}
		else
		{
			if(mbLog){
	...
	void iRenderFunctions::ClearFrameBuffer(tClearFrameBufferFlag aFlags, bool abUsePosAndSize)
	{
			...
			(mpCurrentRenderTarget->mvPos != cVector2l(0,0) || mvRenderTargetSize != mvCurrentFrameBufferSize || mbUseGlobalScissorRect) )
		{
			...
			else
			{
				...
				//Both caches updated, both for the same reason as in SetFrameBuffer: this
				//clips the clear only, but the renderer has to know it happened or its
				//own early-outs start lying about the state of GL.
				mbCurrentScissorActive = true;
				mvCurrentScissorRectPos = mpCurrentRenderTarget->mvPos;
				mvCurrentScissorRectSize = mvRenderTargetSize;
			}
		}
		else
		{
