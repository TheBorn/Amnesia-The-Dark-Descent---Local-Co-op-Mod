...
namespace hpl {
							...
							bool abSendFrameBufferToPostEffects,tRendererCallbackList *apCallbackList)
	{
		...
		// DIAGNOSTIC: per-frame render work, summed over every viewport. Sampled
		// HERE, before EndRendering, because that clears the settings -- reading
		// afterwards just reported 0 lights every frame.
		::ImGuiDebugMenu::AddDiagRenderWork(apSettings->mlNumberOfLightsRendered,
											apSettings->mlNumberOfOcclusionQueries);
	...
	*/
	...
	void iRenderer::RenderShadowMap(iLight *apLight, iFrameBuffer *apShadowBuffer)
	{
		...
		// DIAGNOSTIC: the shadow-map cache (mvShadowMapData) lives on the
		// RENDERER, not the viewport, and IncRenderFrameCount() ticks once per
		// cScene::Render -- not once per viewport. So with the split active,
		// every viewport evicts shadow maps the previous one just rendered.
		// Compare this counter with co-op on vs off, and watch whether it
		// climbs the longer you stay in one map.
		::ImGuiDebugMenu::AddDiagShadowMapRender();
		...
		//A shadow map is a fixed-resolution offscreen buffer with its own
		//light-space projection -- the screen's render scale has no meaning in
		//it. Left scaled, the casters rasterise into the top-left sub-rect while
		//the lighting pass still projects across the whole map, and every spot
		//light's shadow comes out shifted and stretched. Restored at the bottom.
		cVector2f vSavedRenderScale = GetRenderScale();
		SetRenderScale(cVector2f(1.0f, 1.0f));
		...
		//Back to the screen's scale for whatever renders next.
		SetRenderScale(vSavedRenderScale);
	...
	void iRenderer::UpdateQuadVertexUV(iVertexBuffer *apVtxBuffer, const cVector2f& avMinUV, const cVector2f& avMaxUV,
										bool abInvertY, bool abCallUpdate)
	{
		int lVtxStride = apVtxBuffer->GetElementNum(eVertexBufferElement_Texture0);
		float *pUV = apVtxBuffer->GetFloatArray(eVertexBufferElement_Texture0);

		//Same winding and same Y flip as CreateQuadVertexBuffer, so writing the
		//original values back over these is exactly a no-op.
		float fMinUV_Y = abInvertY ? avMaxUV.y : avMinUV.y;
		float fMaxUV_Y = abInvertY ? avMinUV.y : avMaxUV.y;

		pUV[0*lVtxStride +0] = avMinUV.x;	pUV[0*lVtxStride +1] = fMinUV_Y;
		pUV[1*lVtxStride +0] = avMaxUV.x;	pUV[1*lVtxStride +1] = fMinUV_Y;
		pUV[2*lVtxStride +0] = avMaxUV.x;	pUV[2*lVtxStride +1] = fMaxUV_Y;
		pUV[3*lVtxStride +0] = avMinUV.x;	pUV[3*lVtxStride +1] = fMaxUV_Y;

		if(abCallUpdate) apVtxBuffer->UpdateData(eVertexElementFlag_Texture0, false);
	}
