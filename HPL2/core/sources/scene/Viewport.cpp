...
namespace hpl {
	...
	{
		// NOTE: the original code compared against begin() instead of end(),
		// so this loop never executed and viewport callbacks never fired —
		// a bug present since the original release (the engine itself never
		// used these callbacks, so it went unnoticed).
